...
```mermaid
flowchart LR
    PROBLEM[Changing business and threat conditions] --> MISSION[Anticipate, secure, and simplify business experience]
    MISSION --> STRATEGY[Zero trust and AI-forward strategy]
    STRATEGY --> PLATFORM[Platform capabilities]
    PLATFORM --> WORK[Customer adoption and operations]
    WORK --> OUTCOMES[Agility, efficiency, resilience, and security]
    OUTCOMES --> LEARN[Evidence and learning]
    LEARN --> STRATEGY
```
...
```mermaid
flowchart TB
    subgraph LEGACY[Network-centric pattern]
        USER1[User] --> VPN1[VPN or perimeter]
        VPN1 --> NETWORK[Trusted network reach]
        NETWORK --> APP1[Many reachable resources]
    end
    subgraph ZERO[Resource-centric pattern]
        USER2[User or workload] --> VERIFY[Verify identity and context]
        VERIFY --> POLICY[Evaluate destination, risk, and policy]
        POLICY --> APP2[Broker approved connection to one resource]
    end
```
...
```mermaid
flowchart LR
    PLATFORM[Zero Trust Exchange platform] --> SECURE[Secure]
    PLATFORM --> SIMPLIFY[Simplify]
    PLATFORM --> TRANSFORM[Transform]
    SECURE --> S1[Reduce threat and data-loss risk]
    SIMPLIFY --> S2[Reduce complexity and operating overhead]
    TRANSFORM --> S3[Enable cloud, workforce, workload, and branch agility]
    S1 --> VALUE[Customer value]
    S2 --> VALUE
    S3 --> VALUE
```
...
```mermaid
sequenceDiagram
    participant Entity as User, Device, or Workload
    participant Platform as Zero Trust Exchange
    participant Signals as Identity and Context Sources
    participant Resource as Approved Application
    Entity->>Platform: Request access to a destination
    Platform->>Signals: Verify identity and context
    Signals-->>Platform: Return current signals
    Platform->>Platform: Assess risk and evaluate policy
    alt Approved
        Platform->>Resource: Broker a narrow connection
        Resource-->>Entity: Deliver authorized service
    else Not approved
        Platform-->>Entity: Block, limit, or require another step
    end
```
...
```mermaid
flowchart TD
    AI[Artificial intelligence in the enterprise] --> PRODUCT[Product capability]
    AI --> ATTACKER[Attacker force multiplier]
    AI --> EMPLOYEE[Employee workflow]
    AI --> GOVERNANCE[Governance concern]
    PRODUCT --> P[Speed and context with validation]
    ATTACKER --> A[New scale, speed, and attack paths]
    EMPLOYEE --> E[Productivity and learning]
    GOVERNANCE --> G[Data, authority, evaluation, and audit]
```
...
```mermaid
flowchart LR
    GOAL[Attacker goal] --> RECON[Accelerated reconnaissance]
    RECON --> LURE[Scaled and tailored social engineering]
    LURE --> ACCESS[Credential or application access attempt]
    ACCESS --> ADAPT[Rapid variation and adaptation]
    ADAPT --> IMPACT[Potential business impact]
    DEFENSE[Identity, data, network, endpoint, and human controls] --> RECON
    DEFENSE --> LURE
    DEFENSE --> ACCESS
    DEFENSE --> ADAPT
```
...
```mermaid
flowchart TD
    USE[Proposed AI use case] --> DATA{Approved data and purpose?}
    DATA -->|No| REDESIGN[Redesign or reject]
    DATA -->|Yes| AUTH{User and tool authorized?}
    AUTH -->|No| REDESIGN
    AUTH -->|Yes| TEST[Evaluate accuracy, safety, privacy, and bias]
    TEST --> RISK{Risk within tolerance?}
    RISK -->|No| CONTROL[Add controls or narrow scope]
    CONTROL --> TEST
    RISK -->|Yes| DEPLOY[Deploy with human authority and logging]
    DEPLOY --> MONITOR[Monitor errors, drift, abuse, and outcomes]
...
```mermaid
flowchart TD
    CULTURE[Published culture framework] --> COMMIT[Commitments]
    COMMIT --> MISSION[Mission]
    COMMIT --> OUTCOME[Outcome]
    COMMIT --> EACH[Each other]
    CULTURE --> BEHAVIOR[Ways of working]
    BEHAVIOR --> OWN[Ownership and collaboration]
    BEHAVIOR --> TRUST[Trust through outcomes and impact]
    BEHAVIOR --> CHALLENGE[Challenge and ongoing feedback]
    OWN --> EXECUTION[Clear, accountable execution]
    TRUST --> EXECUTION
    CHALLENGE --> EXECUTION
```
...
```mermaid
flowchart LR
    ACTIVITY[Activity: run workshop] --> OUTPUT[Output: agreed workflow]
    OUTPUT --> OUTCOME[Outcome: analysts use workflow correctly]
    OUTCOME --> IMPACT[Impact: faster, better-prioritized response]
    IMPACT --> EVIDENCE[Evidence and learning]
    EVIDENCE --> ACTIVITY
```
...
```mermaid
flowchart TD
    COMMIT[Make a clear commitment] --> EXECUTE[Execute and expose blockers]
    EXECUTE --> UPDATE[Update before surprise]
    UPDATE --> VALIDATE[Validate result with evidence]
    VALIDATE --> CLOSE[Close the loop]
    CLOSE --> LEARN[Record learning and improve]
    LEARN --> COMMIT
```
...
```mermaid
flowchart TD
    REQUEST[Customer request] --> OUTCOME[Clarify desired outcome and impact]
    OUTCOME --> EVIDENCE[Gather evidence and constraints]
    EVIDENCE --> SAFE{Requested path safe and effective?}
    SAFE -->|Yes| EXECUTE[Agree owner, timing, and validation]
    SAFE -->|No| OPTIONS[Explain risk and offer alternatives]
    OPTIONS --> DECIDE[Customer and owners make informed decision]
```
...
sequenceDiagram
    ...
    A->>B: Separate fact, assumption, and concern
    B-->>A: Explain competing evidence and constraint
    A->>B: Test options and consequences
    A->>O: Record recommendation and dissent
    O-->>A: Make decision
    A->>B: Commit to execution unless unsafe or unethical
    A->>O: Reopen only with material new evidence
```
...
```mermaid
flowchart TD
    NEED[Urgent customer need] --> SCOPE[Choose smallest meaningful scope]
    SCOPE --> GUARD[Set quality, safety, and rollback gates]
    GUARD --> BUILD[Build or act quickly]
    BUILD --> TEST[Test with representative evidence]
    TEST --> RELEASE{Gate passed?}
    RELEASE -->|No| FIX[Repair or narrow]
    FIX --> TEST
    RELEASE -->|Yes| USE[Release to bounded use]
    USE --> LEARN[Measure and iterate]
```
...
```mermaid
flowchart LR
    OUTCOME[Customer outcome] --> ACCOUNTABLE[One accountable owner]
    ACCOUNTABLE --> WORK[Responsible workstreams]
    WORK --> CONSULT[Consulted specialists]
    WORK --> INFORM[Informed stakeholders]
    TSM[TSM owns continuity] --> ACCOUNTABLE
    TSM --> WORK
    TSM --> CONSULT
    TSM --> INFORM
```
...
```mermaid
flowchart TD
    CLEAR[Clear outcome and standard] --> SKILL[Right skills and roles]
    SKILL --> VOICE[Relevant voices and dissent]
    VOICE --> EXECUTE[Focused execution]
    EXECUTE --> FEEDBACK[Direct feedback and evidence]
    FEEDBACK --> LEARN[Learning and recognition]
    LEARN --> CLEAR
```
...
```mermaid
flowchart TD
    ISSUE[Issue or request appears] --> IMPACT{Material customer or security impact?}
    IMPACT -->|No| ROUTINE[Prioritize through normal plan]
    IMPACT -->|Yes| FACTS[State verified facts and unknowns]
    FACTS --> AUTH{Do I have authority and safe path?}
    AUTH -->|Yes| ACT[Take bounded action and validate]
    AUTH -->|No| OWNER[Engage accountable owner with options]
    OWNER --> DISAGREE{Material unresolved disagreement?}
    DISAGREE -->|No| COMMIT[Execute and report]
    DISAGREE -->|Yes| CHALLENGE[Challenge with evidence and consequence]
    CHALLENGE --> DECISION{Decision safe, legal, and ethical?}
    DECISION -->|Yes| COMMIT
    DECISION -->|No| ESCALATE[Use formal escalation or stop-work path]
```
...
```mermaid
flowchart TD
    CHANGE[Proposed urgent change] --> CONSEQ{Could failure cause major disruption or exposure?}
    CONSEQ -->|No| FAST[Use lightweight review and monitor]
    CONSEQ -->|Yes| REV{Is the action reversible and scopeable?}
    REV -->|Yes| PILOT[Run bounded pilot with rollback]
    REV -->|No| MIT{Is there a safer temporary mitigation?}
    MIT -->|Yes| TEMP[Apply mitigation and complete review]
    MIT -->|No| AUTH[Escalate risk decision to authorized owner]
    PILOT --> EVIDENCE[Validate evidence before expansion]
    TEMP --> EVIDENCE
```
...
```mermaid
flowchart TD
    ASK[Customer asks for action] --> WHY[Clarify desired outcome and urgency]
    WHY --> FIT{Does requested action address the outcome?}
    FIT -->|Unknown| TEST[Gather evidence or run a bounded test]
    FIT -->|Yes| RISK{Risk and authority acceptable?}
    FIT -->|No| ALT[Offer evidence-based alternatives]
    RISK -->|Yes| PLAN[Agree owner, timing, and validation]
    RISK -->|No| ALT
    ALT --> DECIDE[Record informed decision]
```
...
```mermaid
flowchart LR
    ROLE[Current job description] --> THEMES[Extract role and culture themes]
    COMPANY[Official company and culture pages] --> THEMES
    PRODUCT[Official product pages] --> CLAIMS[Record documented claims]
    THEMES --> MAP[Map to candidate evidence]
    CLAIMS --> CAVEAT[Mark volatility and caveats]
    MAP --> GAPS[Identify unsupported gaps]
    CAVEAT --> SCRIPT[Build concise why and scenario answers]
    GAPS --> SCRIPT
    SCRIPT --> VERIFY[Recheck before interview]
```
...
```mermaid
flowchart LR
    ANSWER[Practice answer] --> FACT[Fact and claim label]
    ANSWER --> ACTION[Observable behavior]
    ANSWER --> OUTCOME[Measured outcome]
    ANSWER --> TENSION[Tradeoff or constraint]
    ANSWER --> LEARN[Learning and next proof]
    FACT --> SCORE[Interview score]
    ACTION --> SCORE
    OUTCOME --> SCORE
    TENSION --> SCORE
    LEARN --> SCORE
