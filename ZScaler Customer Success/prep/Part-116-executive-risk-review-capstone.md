...
```mermaid
flowchart LR
    P112[Part 112 synthetic data quality] --> TECH[Technical review]
    P113[Part 113 synthetic priorities] --> TECH
    P114[Part 114 synthetic escalation] --> TECH
    P115[Part 115 fictional outcomes] --> TECH
    TECH --> REG[Risk register]
    REG --> DASH[Executive dashboard]
    REG --> ROAD[Mitigation roadmap]
    DASH --> DECK[Executive deck]
    ROAD --> DECK
    DECK --> DEC[Decision requests]
    DEC --> OUT[Next-quarter outcome contracts]
    OUT --> VAL[Validation and residual risk]
```
...
```mermaid
flowchart LR
    FIND[Finding with provenance] --> CONTEXT[Asset identity exposure business context]
    CONTEXT --> SCEN[Credible threat or failure scenario]
    SCEN --> OBJ[Objective at risk]
    OBJ --> CONSEQ[Potential consequence]
    CTRL[Current control evidence] --> SCEN
    QUAL[Quality freshness confidence] --> CONTEXT
    CONSEQ --> RISK[Bounded risk statement]
    RISK --> OPT[Mitigation options]
    OPT --> DEC[Authorized decision]
```
...
```mermaid
flowchart TB
    RAW[Immutable synthetic source fixtures] --> RULE[Versioned mapping and scoring rules]
    RULE --> OBS[Reconciled observations]
    OBS --> RISK[Risk register entries]
    RISK --> VIS[Dashboard measures]
    RISK --> MIT[Mitigation options]
    VIS --> SLIDE[Executive statements]
    MIT --> SLIDE
    SLIDE --> DEC[Decision register]
    DEC --> PLAN[Roadmap commitments]
    PLAN --> TEST[Acceptance evidence]
    TEST --> RES[Residual-risk review]
```
...
```mermaid
flowchart TD
    Q[Review question] --> POP[Define population and reporting cut]
    POP --> REC[Reconcile source counts and freshness]
    REC --> ENT[Validate entity and ownership context]
    ENT --> PRI[Explain priority and confidence]
    PRI --> CTRL[Inspect control evidence and exceptions]
    CTRL --> FLOW[Trace remediation and escalation workflow]
    FLOW --> CHAL[Challenge interpretation and alternatives]
    CHAL --> APP[Approve bounded technical observation]
```
...
```mermaid
flowchart LR
    SRC[Source rows 8] --> ACC[Accepted contextual rows 7]
    SRC --> QUAR[Quarantined unresolved row 1]
    ACC --> MET[Dashboard denominator 7]
    QUAR --> QUAL[Quality and uncertainty panel]
    MET --> TREND[Outcome interpretation]
    QUAL --> TREND
    TREND --> NOTE[Headline plus limitation]
```
...
```mermaid
flowchart TD
    OBS[Observation] --> CONF{Confidence adequate}
    CONF -- No --> QUAR[Quarantine or qualify]
    QUAR --> EVID[Request discriminating evidence]
    CONF -- Yes --> SCEN{Scenario and objective explicit}
    SCEN -- No --> REWRITE[Rewrite risk statement]
    SCEN -- Yes --> CTRL[Assess control evidence]
    CTRL --> OPT[Develop options]
    OPT --> OWNER{Authorized owner present}
    OWNER -- No --> ESC[Escalate decision ownership]
    OWNER -- Yes --> REG[Register treatment and residual hypothesis]
```
...
```mermaid
flowchart LR
    EXEC[Executive Decisions page] --> RISK[Risk and Mitigation page]
    RISK --> QUAL[Data and Workflow Confidence page]
    EXEC --> D1[Decision cards]
    EXEC --> O1[Outcome contracts]
    RISK --> R1[Risk register drill]
    RISK --> M1[Roadmap status]
    QUAL --> Q1[Freshness completeness linkage]
    QUAL --> W1[Ticket and validation reconciliation]
    Q1 --> ROW[Sanitized synthetic row evidence]
    W1 --> ROW
```
...
```mermaid
sequenceDiagram
    participant E as Fictional executive
    participant D as Dashboard
    participant R as Risk register
    participant T as Technical evidence
    E->>D: Select material-risk card
    D->>R: Open scenario, owner, treatment, confidence
    R->>T: Request contributing observations
    T-->>R: Return synthetic rows, rules, freshness, limits
    R-->>D: Return bounded explanation
    D-->>E: Show decision, options, residual uncertainty
```
...
```mermaid
flowchart LR
    C[Context and objectives] --> CH[What changed or was learned]
    CH --> CON[Consequence and uncertainty]
    CON --> OPT[Options and tradeoffs]
    OPT --> ASK[Decision request]
    ASK --> COM[Owner commitment]
    COM --> VAL[Acceptance evidence]
```
...
```mermaid
flowchart TD
    ASK[Decision request] --> AUTH{Correct authority present}
    AUTH -- No --> ROUTE[Route and set checkpoint]
    AUTH -- Yes --> OPT[Compare options and tradeoffs]
    OPT --> EVID{Evidence sufficient}
    EVID -- No --> DEF[Defer boundedly and request evidence]
    EVID -- Yes --> CHOOSE[Record selected option and rationale]
    CHOOSE --> OWN[Assign owner dependency due basis]
    OWN --> ACCEPT[Define acceptance and residual review]
```
...
```mermaid
gantt
    title Synthetic NMH next-quarter mitigation roadmap
    dateFormat  YYYY-MM-DD
    axisFormat  %b %d
    section Trust foundation
    Scope and decision read-back        :a1, 2026-08-25, 5d
    Ownership and source repair         :a2, after a1, 20d
    section Treatment
    Anchor-case validation plans        :b1, 2026-09-01, 15d
    Controlled mitigation and evidence  :b2, after b1, 30d
    section Workflow
    Closure-validation reconciliation   :c1, 2026-09-05, 30d
    Changed-case training and retry     :c2, 2026-09-10, 25d
    section Governance
    Bounded automation pilot            :d1, 2026-10-01, 25d
    Executive residual-risk review      :d2, after d1, 10d
```
...
```mermaid
flowchart LR
    ACT[Activity completed] --> CAP{Capability demonstrated}
    CAP -- No --> PRACT[Practice and correct]
    CAP -- Yes --> BEH{Repeated behavior observed}
    BEH -- No --> PILOT[Run bounded pilot]
    BEH -- Yes --> EFFECT{Operational effect reconciled}
    EFFECT -- No --> CHECK[Check data workflow confounders]
    EFFECT -- Yes --> ACCEPT{Authorized outcome acceptance}
    ACCEPT -- No --> REVIEW[Resolve evidence or expectation]
    ACCEPT -- Yes --> NEXT[Record residual risk and next outcome]
```
...
```mermaid
flowchart TD
    P111[Part 111 gates] --> INPUT[Only sanitized synthetic inputs]
    INPUT --> CUT[Fixed reporting cut]
    CUT --> TOOLS[Approved local authoring tools]
    TOOLS --> IDS[Artifact IDs and visible labels]
    IDS --> REVIEW[Reviewer role and authority boundary]
    REVIEW --> READY{All prerequisites evidenced}
    READY -- No --> STOP[Repair before lab]
    READY -- Yes --> LAB[Begin capstone]
```
...
```mermaid
flowchart TD
    E1[Charter and manifest] --> E2[Technical evidence spine]
    E2 --> E3[Challenge review]
    E3 --> E4[Risk register]
    E4 --> E5[Dashboard]
    E4 --> E6[Executive deck]
    E5 --> E7[Decision and action registers]
    E6 --> E7
    E7 --> E8[Roadmap]
    E8 --> E9[Outcome contracts]
    E9 --> E10[Executive rehearsal]
    E10 --> E11[Contrarian review]
    E11 --> E12[Follow-up validation release]
```
...
```mermaid
flowchart TD
    BAD[Review does not support a decision] --> TRACE{Headline traces to evidence}
    TRACE -- No --> SPINE[Repair evidence spine]
    TRACE -- Yes --> QUAL{Quality and denominator visible}
    QUAL -- No --> DICT[Repair metric contract]
    QUAL -- Yes --> RISK{Scenario objective controls explicit}
    RISK -- No --> REWRITE[Rewrite risk statement]
    RISK -- Yes --> AUTH{Decision and authority explicit}
    AUTH -- No --> ASK[Repair decision package]
    AUTH -- Yes --> VAL{Outcome has acceptance evidence}
    VAL -- No --> CONTRACT[Repair roadmap and outcome contract]
    VAL -- Yes --> REHEARSE[Rehearse and challenge]
```
...
```mermaid
flowchart TD
    INV[Inventory A01-A14 drafts exports caches] --> REAL{Any real or unclear-origin content}
    REAL -- Yes --> STOP[Stop exclude and follow applicable policy]
    REAL -- No --> LABEL{Synthetic label visible in body}
    LABEL -- No --> FIX[Correct artifact and export]
    LABEL -- Yes --> META[Inspect metadata comments paths hidden pages]
    META --> LINKS[Test source and evidence links]
    LINKS --> SELECT[Select intentional release subset]
    SELECT --> DELETE[Remove exact temporary synthetic copies]
    DELETE --> LOG[Record disposition and final hash/version]
```
...
```mermaid
flowchart TD
    GATE[Part 111 blocking gates] --> TECH[Technical trace review]
    TECH --> RISK[Risk narrative review]
    RISK --> EXEC[Dashboard and deck review]
    EXEC --> DEC[Decision and roadmap review]
    DEC --> OUT[Outcome-contract review]
    OUT --> CHAL[Changed-case and contrarian review]
    CHAL --> SCORE{At least 85 and no critical defect}
    SCORE -- No --> REWORK[Correct version and rerun]
    SCORE -- Yes --> READY[Portfolio-ready synthetic capstone]
```
