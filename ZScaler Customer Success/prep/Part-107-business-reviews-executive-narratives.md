...
```mermaid
flowchart LR
    OBJ[Customer objectives risks and prior decisions] --> EVID[Validated technical data workflow support and sentiment evidence]
    EVID --> INTERP[Interpret definition scope trend confidence and cause limits]
    INTERP --> NARR[Outcome-first narrative]
    NARR --> ASK[Decision options owner and need-by basis]
    ASK --> DECK[Concise executive slides]
    EVID --> APP[Technical appendix and source ledger]
    DECK --> REVIEW[QBR or EBR discussion]
    APP --> REVIEW
    REVIEW --> DEC[Decision action owner and follow-up]
```
...
```mermaid
flowchart TB
    NEED[Review need] --> AUD{Primary audience and decisions?}
    AUD -->|Operational and quarterly execution| QBR[QBR]
    AUD -->|Executive priorities risks and sponsorship| EBR[EBR]
    QBR --> SHARED[Shared objective evidence and action spine]
    EBR --> SHARED
    SHARED --> MAIN[Concise main narrative]
    SHARED --> APP[Technical and analytical appendix]
    MAIN --> DEC[Decisions and roadmap]
    APP --> DEC
```
...
```mermaid
flowchart LR
    OBJ[Objective] --> GAP[Variance risk or opportunity]
    GAP --> CAUSE[Evidence and bounded mechanism]
    CAUSE --> OPT[Options and tradeoffs]
    OPT --> REC[Conditional recommendation]
    REC --> ASK[Decision owner and need-by basis]
    ASK --> PLAN[Action validation and residual]
```
...
```mermaid
sequenceDiagram
    participant T as TSM or review owner
    participant D as Data and technical owners
    participant S as Support Product and specialists
    participant A as Account team
    participant C as Customer decision owners
    T->>D: Request metrics definitions evidence cut and exceptions
    D-->>T: Return source data quality and interpretation
    T->>S: Validate product support and roadmap facts
    S-->>T: Return approved bounded status
    T->>A: Pre-brief objectives risks asks and boundaries
    A-->>T: Correct commercial and account context
    T->>C: Send concise pre-read and decision requests
    C-->>T: Raise questions or corrections before review
```
...
```mermaid
flowchart TB
    CHART[Chart or score] --> DEF[Definition grain numerator denominator]
    DEF --> SCOPE[Population filters cohort and exclusions]
    SCOPE --> TIME[Event effective refresh and comparison time]
    TIME --> QUAL[Source quality coverage and confidence]
    QUAL --> TREND[Trend variance distribution and anomaly]
    TREND --> CAUSE[Established mechanism alternatives and unknowns]
    CAUSE --> DEC[Meaning decision and next evidence]
```
...
```mermaid
flowchart LR
    OUT[Outcome or risk] --> EV[Evidence and confidence]
    EV --> MEAN[Meaning mechanism and limitation]
    MEAN --> DEC[Decision options and recommendation]
    DEC --> NEXT[Owner action validation residual]
```
...
```mermaid
flowchart LR
    S1[1 Purpose objectives and decisions] --> S2[2 Executive outcome summary]
    S2 --> S3[3 Progress and value evidence]
    S3 --> S4[4 Health adoption and support]
    S4 --> S5[5 Material risks and bad news]
    S5 --> S6[6 Decisions and options]
    S6 --> S7[7 Next-period roadmap]
    S7 --> S8[8 Commitments and close]
    S8 --> APP[Technical appendix]
```
...
```mermaid
flowchart LR
    TECH[Validated technical condition] --> SCOPE[Affected service data users and obligation]
    SCOPE --> PATH[Plausible path and preconditions]
    PATH --> CTRL[Existing control evidence and gaps]
    CTRL --> CONSEQ[Operational strategic legal safety or financial consequence]
    CONSEQ --> OPT[Options tradeoffs and urgency]
    OPT --> GOV[Authorized decision oversight and residual]
```
...
```mermaid
flowchart TB
    NEED[Customer requirement] --> CURRENT{Current verified capability meets it?}
    CURRENT -- Yes --> PLAN[Adopt validate and measure]
    CURRENT -- Partial --> ALT[Workaround alternative scope or services]
    CURRENT -- No --> REQ[Record product feedback through approved route]
    REQ --> STATUS[Use only approved attributable status]
    STATUS --> DEP{Can plan succeed without future capability?}
    DEP -- Yes --> BASE[Use current-state plan and revisit on trigger]
    DEP -- No --> DEC[Explicit customer decision to rescope defer or accept dependency]
```
...
```mermaid
flowchart LR
    BAD[Material unfavorable evidence] --> VALID[Validate scope impact and confidence]
    VALID --> EARLY[Notify correct owner early]
    EARLY --> FACT[State facts unknowns and what changed]
    FACT --> ACT[Containment workstreams support and decisions]
    ACT --> CAD[Cadence and next evidence milestone]
    CAD --> REC[Recovery validation]
    REC --> LEARN[Closure PIR and roadmap impact]
```
...
sequenceDiagram
    ...
    participant O as Technical and outcome owners
    participant A as Account team
    F->>E: Open purpose reporting cut and decisions
    F->>O: Confirm objective and evidence headlines
    O-->>F: Correct facts or raise exceptions
    F->>E: Present risks options and recommendations
    E-->>F: Decide defer or request evidence
    F->>A: Confirm roadmap commercial and support boundaries
    F->>E: Read back decisions actions and residual
    F-->>O: Publish record and validation checkpoints
```
...
```mermaid
flowchart TB
    ITEM[Candidate content] --> DEC{Changes an executive understanding decision risk or commitment?}
    DEC -- No --> ASYNC[Async report or omit]
    DEC -- Yes --> EVID{Validated and appropriately shareable?}
    EVID -- No --> HOLD[Mark unknown verify or restrict]
    EVID -- Yes --> ONE{Can one clear conclusion be stated?}
    ONE -- No --> WORK[Resolve analysis or split message]
    ONE -- Yes --> MAIN[Main slide with evidence and ask]
    MAIN --> DETAIL[Methods and detail in appendix]
```
...
```mermaid
flowchart TB
    ASK[Future capability question] --> CUR{Current verified capability meets need?}
    CUR -- Yes --> CURRENT[Discuss current plan and validation]
    CUR -- No --> AUTH{Approved attributable future status available?}
    AUTH -- No --> UNKNOWN[State unknown; submit request and use alternatives]
    AUTH -- Yes --> QUAL[Preserve exact status qualifiers source and date]
    QUAL --> DEP{Customer plan depends on it?}
    DEP -- No --> TRIG[Track revisit trigger]
    DEP -- Yes --> DECIDE[Customer decides rescope defer alternative or risk]
```
...
```mermaid
flowchart TB
    SIGNAL[Unfavorable signal] --> VALID{Material enough for decision or impact?}
    VALID -- No --> TRACK[Track in normal cadence]
    VALID -- Yes --> ACTIVE{Active incident or critical impact?}
    ACTIVE -- Yes --> ESC[Use escalation cadence immediately]
    ACTIVE -- No --> OWNER[Notify sponsor and affected owner early]
    ESC --> FACT[Impact facts unknowns actions next checkpoint]
    OWNER --> FACT
    FACT --> ASK{Decision needed?}
    ASK -- Yes --> OPT[Options recommendation owner need-by]
    ASK -- No --> FOLLOW[Action validation and review]
    OPT --> FOLLOW
```
