...
```mermaid
flowchart LR
    AUTH[Written authority and local scope] --> SYN[Synthetic deterministic inputs]
    SYN --> RUN[Least privileged local run]
    RUN --> OBS[Observed evidence]
    OBS --> LIMIT[Limits and alternative explanations]
    LIMIT --> CLAIM[Honest claim label]
    CLAIM --> REVIEW[Reproduce review and improve]
```
...
```mermaid
flowchart TD
    IDEA[Proposed lab action] --> OWN{Created locally for this lab}
    OWN -- No --> AUTH{Written specific authorization}
    OWN -- Yes --> DATA{Only synthetic or approved data}
    AUTH -- No --> STOP[Do not perform action]
    AUTH -- Yes --> DATA
    DATA -- No --> NARROW[Redesign with synthetic minimized data]
    DATA -- Yes --> PRIV{Least privilege and reversible}
    PRIV -- No --> NARROW
    PRIV -- Yes --> NET{No unintended external target}
    NET -- No --> NARROW
    NET -- Yes --> RUN[Record scope run and cleanup]
```
...
```mermaid
flowchart LR
    PLAN[Plan expected target privilege output] --> PRE[Preflight path identity and network]
    PRE --> EXEC[Execute smallest local step]
    EXEC --> MATCH{Observed behavior matches plan}
    MATCH -- Yes --> RECORD[Record evidence and continue]
    MATCH -- No --> HALT[Stop preserve minimal evidence]
    HALT --> REVIEW[Explain difference and redesign]
    REVIEW --> PLAN
```
...
```mermaid
flowchart TB
    RAW[01 raw immutable synthetic inputs] --> STAGE[02 staged normalized data]
    STAGE --> ANALYSIS[03 SQL notebooks or query files]
    ANALYSIS --> OUTPUT[04 tables charts and dashboards]
    OUTPUT --> NARRATIVE[05 executive technical and interview narratives]
    RAW --> META[00 manifest scope dictionary sources]
    STAGE --> META
    ANALYSIS --> META
    OUTPUT --> META
    NARRATIVE --> META
    META --> REVIEW[06 validation and reflection]
    REVIEW --> RELEASE[07 sanitized portfolio release]
```
...
```mermaid
flowchart LR
    CONTEXT[State synthetic context] --> ACTION[Name personal action]
    ACTION --> RESULT[Report observed result]
    RESULT --> LIMIT[State what it does not prove]
    LIMIT --> PROD[Name production validation]
```
...
```mermaid
flowchart LR
    SRC[Source row and source timestamp] --> MAP[Mapping rule and version]
    MAP --> CLEAN[Quality and normalization]
    CLEAN --> DERIVE[Derived field formula]
    DERIVE --> QUERY[Saved query and reporting cut]
    QUERY --> VIEW[Chart table or narrative]
    VIEW --> CLAIM[Bounded portfolio claim]
```
...
```mermaid
flowchart TD
    START[Choose local analysis path] --> HASDB{Approved SQL tool already available}
    HASDB -- SQLite --> SQLITE[Single local database file]
    HASDB -- DuckDB --> DUCK[Local analytical database file]
    HASDB -- PostgreSQL --> PG[Local database and standard user]
    HASDB -- No --> SHEET[Spreadsheet tables pivots and charts]
    SQLITE --> SAME[Use shared schema and evidence contract]
    DUCK --> SAME
    PG --> SAME
    SHEET --> SAME
    SAME --> BI{Optional local BI already approved}
    BI -- Yes --> DASH[Build local dashboard]
    BI -- No --> REPORT[Saved queries tables and charts]
```
...
```mermaid
flowchart TD
    SPEC[Scenario specification] --> SEED[Fixed seed and reporting cut]
    SEED --> ENT[Create fictional entities]
    ENT --> REL[Create controlled relationships]
    REL --> DEF[Inject named quality defects]
    DEF --> EXPORT[Write versioned CSV or JSON]
    EXPORT --> ASSERT[Assert counts keys ranges and defect inventory]
    ASSERT --> FREEZE[Freeze raw synthetic inputs]
```
...
```mermaid
flowchart TD
    NEED[Need to answer a network question] --> STATIC{Can static synthetic evidence answer it}
    STATIC -- Yes --> USE[Use supplied offline fixture]
    STATIC -- No --> AUTH{Specific authorized isolated lab exists}
    AUTH -- No --> REDESIGN[Redesign question or mark unknown]
    AUTH -- Yes --> MIN[Minimize interface host duration fields]
    MIN --> CAP[Capture only isolated loopback fixture]
    CAP --> REDACT[Review redact restrict retain briefly]
    REDACT --> DELETE[Delete raw copy when no longer required]
```
...
sequenceDiagram
    ...
    A->>M: Record seed schema cut and versions
    A->>T: Load frozen synthetic inputs
    A->>T: Run saved transformations and queries
    T-->>A: Produce counts tables and warnings
    A->>M: Record outputs checks and limitations
    R->>M: Read scope and reproduction steps
    R->>T: Repeat with same inputs and settings
    T-->>R: Produce materially comparable results
    R->>M: Record match variance or failure
```
...
```mermaid
flowchart LR
    CHARTER[Scope and safety charter] --> DATA[Dictionary and frozen input]
    DATA --> QUERY[Saved transformation and query]
    QUERY --> RESULT[Reconciled result]
    RESULT --> CLAIM[Claim and limitation ledger]
    CLAIM --> RUBRIC[Validation and reflection]
    RUBRIC --> RELEASE[Sanitized release subset]
```
...
```mermaid
flowchart TD
    FAIL[Validation mismatch] --> INPUT{Raw count and hash match}
    INPUT -- No --> RAW[Restore frozen synthetic input]
    INPUT -- Yes --> LOAD{Import types and headers correct}
    LOAD -- No --> FIXLOAD[Correct import configuration]
    LOAD -- Yes --> QUERY{Saved query matches version}
    QUERY -- No --> RESTORE[Use recorded query]
    QUERY -- Yes --> FILTER{Reporting cut filters and joins visible}
    FILTER -- No --> FIXFILTER[Correct and document]
    FILTER -- Yes --> EXPECT{Expected result specification correct}
    EXPECT -- No --> VERSION[Version specification and explain]
    EXPECT -- Yes --> ESC[Record unresolved defect and stop claim]
```
...
```mermaid
flowchart TD
    INV[Inventory generated artifacts] --> CLASS[Classify retain release restrict delete]
    CLASS --> REVIEW[Review content metadata links and caches]
    REVIEW --> COPY[Create sanitized release copies]
    COPY --> VERIFY[Open release copies and validate labels]
    VERIFY --> DEL[Delete exact temporary synthetic copies]
    DEL --> CONFIRM[Confirm no real data secrets or captures]
    CONFIRM --> RECORD[Record disposition date and owner]
```
...
```mermaid
flowchart TD
    REVIEW[Review portfolio] --> GATE{All five safety gates pass}
    GATE -- No --> BLOCK[Blocked from release regardless of score]
    GATE -- Yes --> SCORE[Score ten dimensions]
    SCORE --> BAND{Score band}
    BAND -- Below 70 --> REWORK[Rework and rerun]
    BAND -- 70 to 84 --> READY[Portfolio ready with noted gaps]
    BAND -- 85 to 100 --> STRONG[Strong synthetic evidence]
    STRONG --> LIMIT[Still not production experience]
    READY --> LIMIT
```
...
```mermaid
flowchart LR
    P111[Part 111 shared contract] --> P112[Data modeling lab]
    P111 --> P113[Vulnerability prioritization lab]
    P111 --> P114[Connectivity escalation lab]
    P111 --> P115[Discovery onboarding training simulation]
    P111 --> P116[Executive risk capstone]
    P111 --> P117[Complete account capstone]
    P112 --> P116
    P113 --> P116
    P114 --> P117
    P115 --> P117
