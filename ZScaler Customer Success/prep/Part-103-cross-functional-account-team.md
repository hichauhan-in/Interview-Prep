...
```mermaid
flowchart LR
    CUST[Customer outcome and decision] --> TEAM[One account-team fact pattern]
    TEAM --> SALES[Sales and account leadership]
    TEAM --> TSM[Technical Success Management]
    TEAM --> SPEC[Technical specialists and services]
    TEAM --> SUP[Support]
    TEAM --> PROD[Product Management]
    TEAM --> ENG[Engineering and operations]
    SALES --> PLAN[Shared customer success plan]
    TSM --> PLAN
    SPEC --> PLAN
    SUP --> PLAN
    PROD --> PLAN
    ENG --> PLAN
    PLAN --> EVID[Evidence decisions actions and residual]
```
...
```mermaid
flowchart TB
    CUSTOMER[Customer sponsor owners and operators] --> OUT[Customer outcome]
    SALES[Sales/account] --> COMM[Commercial relationship and scope]
    TSM[TSM] --> SUCCESS[Technical success adoption and orchestration]
    ARCH[Technical specialists] --> DESIGN[Architecture and fit]
    PS[Services] --> IMPLEMENT[Contracted delivery]
    SUP[Support] --> INCIDENT[Technical incident path]
    PM[Product] --> DIRECTION[Problem evaluation and approved direction]
    ENG[Engineering/operations] --> BUILD[Technical investigation build and service]
    COMM --> OUT
    SUCCESS --> OUT
    DESIGN --> OUT
    IMPLEMENT --> OUT
...
```mermaid
flowchart TD
    ASK[Customer or internal request] --> CLASS{Technical success implementation incident product commercial or customer decision?}
    CLASS --> AUTH[Identify accountable role and process]
    AUTH --> TSM{What can TSM own?}
    TSM --> ORCH[Clarify coordinate evidence adoption and communication]
    AUTH --> HAND[Handoff specialized decision or work]
    ORCH --> PLAN[Shared plan and checkpoint]
    HAND --> PLAN
```
...
```mermaid
flowchart TB
    OUT[Customer outcomes and success evidence] --> MASTER[Shared account plan]
    TECH[Technical milestones and adoption] --> MASTER
    COMM[Commercial scope and lifecycle] --> MASTER
    CASE[Support cases and mitigations] --> MASTER
    FEED[Product feedback and gaps] --> MASTER
    SERV[Services delivery] --> MASTER
    MASTER --> GOV[Customer and account governance]
    GOV --> DEC[Decisions and coordinated communication]
```
...
```mermaid
sequenceDiagram
    participant F as Sending role
    participant R as Receiving role
    participant T as TSM/account coordinator
    participant C as Customer owner
    F->>T: Provide outcome scope facts evidence unknowns and ask
    T->>R: Route through correct process with secure evidence
    R-->>T: Accept clarify reject or redirect with reason
    T->>C: Confirm owner current bounded status and checkpoint
    R->>R: Perform authorized work
    R-->>T: Return finding decision limitation and next step
    T->>C: Integrate result into shared plan and validate understanding
```
...
```mermaid
flowchart LR
    REQ[Customer requirement] --> TECH[Technical fit and dependency validation]
    REQ --> COMM[Commercial entitlement and service validation]
    TECH --> JOIN[Reconciled requirement-to-scope record]
    COMM --> JOIN
    JOIN --> PLAN[Customer success plan]
    PLAN --> EVID[Adoption outcome and risk evidence]
    EVID --> REVIEW[Technical and commercial review with clear ownership]
```
...
```mermaid
flowchart TD
    ISSUE[Customer issue] --> IMP[Validate impact scope and urgency]
    IMP --> LAYER[Trace architecture and first failed or unknown boundary]
    LAYER --> TYPE{Incident design implementation adoption commercial feedback or security?}
    TYPE --> PATH[Select accountable process and owner]
    PATH --> PACK[Package evidence and exact ask]
    PACK --> ACCEPT[Receive ownership and checkpoint]
    ACCEPT --> PLAN[Integrate mitigation communication and success risk]
```
...
stateDiagram-v2
    ...
    Investigating --> WaitingEvidence
    WaitingEvidence --> Investigating
    Investigating --> Mitigated
    Investigating --> ResolvedCandidate
    Mitigated --> ResolvedCandidate
    ResolvedCandidate --> Validating
    Validating --> Closed: acceptance passes
    Validating --> Investigating: symptom or residual fails
    Closed --> Reopened: recurrence or invalidated evidence
```
...
```mermaid
flowchart TD
    OBS[Observed unmet need or behavior] --> EXP{Current expected behavior established?}
    EXP -- No --> VERIFY[Verify docs capability entitlement configuration and scope]
    EXP -- Yes --> MATCH{Observed behavior deviates reproducibly?}
    MATCH -- Yes --> DEF[Support-led suspected defect path]
    MATCH -- No --> NEED{Customer requirement still unmet?}
    NEED -- Yes --> FEAT[Product feedback/request path]
    NEED -- No --> EDUC[Clarify workflow or documentation]
    VERIFY --> MATCH
```
...
```mermaid
flowchart LR
    NEED[Customer future need] --> CURRENT[Verify current capability and alternatives]
    CURRENT --> FEEDBACK[Record product feedback]
    FEEDBACK --> AUTH{Approved status available to this audience?}
    AUTH -- No --> BOUND[State no approved update]
    AUTH -- Yes --> EXACT[Use exact approved scope caveat and date language]
    EXACT --> PLAN{Can success plan depend on it?}
    PLAN -- No commitment --> ALT[Plan current alternatives and decision trigger]
    PLAN -- Authorized commitment --> DEP[Record accountable commitment and dependencies]
```
...
```mermaid
flowchart TB
    TECH[Technical evidence] --> CUSTDEC[Customer architecture risk and change decision]
    REQ[Requirement and impact] --> PRODDEC[Product evaluation and prioritization]
    CASE[Reproduction and impact] --> SUPDEC[Support investigation process]
    SCOPE[Product and service need] --> COMMDEC[Commercial authority]
    TSM[TSM orchestration] --> TECH
    TSM --> REQ
    TSM --> CASE
    TSM --> SCOPE
    TSM --> PLAN[Shared plan communication and follow-through]
```
...
```mermaid
flowchart TD
    DIFF[Internal difference or ambiguity] --> PRIVATE[Resolve in appropriate internal forum]
    PRIVATE --> FACT[Align facts sources scope and unknowns]
    FACT --> RIGHT[Identify decision right]
    RIGHT --> OPTIONS[Develop customer-relevant options]
    OPTIONS --> DEC[Authorized role decides or states current unknown]
    DEC --> MSG[Coordinate one factual customer message]
    MSG --> RECORD[Update shared plan and source records]
```
...
```mermaid
flowchart LR
    SIGNAL[Customer case feedback adoption or outcome signal] --> CLASS[Classify incident gap documentation workflow commercial or training]
    CLASS --> ROUTE[Route to Support Product Services Sales customer or TSM lane]
    ROUTE --> RESULT[Finding decision mitigation or alternative]
    RESULT --> ACCOUNT[Update success and risk plan]
    RESULT --> SCALE[Approved product runbook training or process learning]
    ACCOUNT --> VALID[Validate customer effect]
    VALID --> SIGNAL
```
...
```mermaid
flowchart TD
    SHARE[Proposed cross-functional evidence share] --> PURPOSE{Required for exact process and decision?}
    PURPOSE -- No --> OMIT[Do not share]
    PURPOSE -- Yes --> CHANNEL{Approved recipient channel and authority?}
    CHANNEL -- No --> FIX[Resolve governance first]
    CHANNEL -- Yes --> MIN{Can content be minimized redacted or aggregated?}
    MIN -- Yes --> SAFE[Use bounded evidence]
    MIN -- No --> FULL[Use approved protected transfer]
    SAFE --> RETAIN[Apply access retention audit and deletion]
    FULL --> RETAIN
```
...
```mermaid
flowchart TD
    FAIL[Cross-functional symptom] --> OUT{Shared customer outcome and fact pattern?}
    OUT -- No --> ALIGN[Align scope facts sources and unknowns]
    OUT -- Yes --> ROLE{Correct accountable lane and decision right?}
    ROLE -- No --> ROUTE[Reclassify and hand off with acceptance]
    ROLE -- Yes --> EVID{Decision-ready evidence and exact ask?}
    EVID -- No --> PACK[Build discriminating packet]
    EVID -- Yes --> COMM{Coordinated customer message and checkpoint?}
    COMM -- No --> MSG[Assign communicator and correct]
    COMM -- Yes --> EXEC[Execute validate update plan and learn]
```
...
```mermaid
flowchart TD
    R[Request or problem] --> C{Commercial terms or entitlement?}
    C -- Yes --> SALES[Sales/account/procurement path]
    C -- No --> I{Supported technical incident?}
    I -- Yes --> SUPPORT[Support path with evidence]
    I -- No --> D{Implementation or contracted delivery?}
    D -- Yes --> SERVICES[Services/customer delivery path]
    D -- No --> F{Unmet current capability requirement?}
    F -- Yes --> PRODUCT[Product feedback plus current alternatives]
    F -- No --> S{Technical success adoption or proactive risk?}
    S -- Yes --> TSM[TSM/customer success plan]
    S -- No --> VERIFY[Clarify scope and authority]
```
...
```mermaid
flowchart TD
    Q[Future capability question] --> AUTH{Authorized source and audience?}
    AUTH -- No --> NONE[State no approved update and verify]
    AUTH -- Yes --> STATUS{Exact approved status scope caveat and date?}
    STATUS -- No --> NONE
    STATUS -- Yes --> SAY[Use exact approved language]
    SAY --> COMMIT{Authorized commitment?}
    COMMIT -- No --> ALT[Plan against current capability and alternatives]
    COMMIT -- Yes --> RECORD[Record commitment owner conditions and dependencies]
```
...
```mermaid
flowchart TD
    E[Issue] --> IMP{Material customer impact or strategic decision?}
    IMP -- No --> NORMAL[Use normal accountable process]
    IMP -- Yes --> READY{Impact scope evidence and exact ask ready?}
    READY -- No --> BUILD[Build minimum packet and mitigation]
    READY -- Yes --> GAP{Current lane lacks authority expertise or progress?}
    GAP -- No --> CAD[Continue with clear cadence]
    GAP -- Yes --> ESC[Escalate to specific authority]
    ESC --> RESULT[Record decision status communication and residual]
```
