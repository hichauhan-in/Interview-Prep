...
```mermaid
flowchart LR
    ROLE[Role outcomes and boundaries] --> COMP[Observable competency model]
    COMP --> BASE[Baseline evidence and gap plan]
    BASE --> LEARN[Study demo and guided practice]
    LEARN --> SHADOW[Observe expert with questions]
    SHADOW --> REVERSE[Perform while expert observes]
    REVERSE --> REVIEW[Rubric feedback and case review]
    REVIEW --> INDEP[Bounded independent contribution]
    INDEP --> SCALE[Knowledge coaching and community contribution]
    SCALE --> IMPROVE[Quality outcome and ramp review]
    IMPROVE --> COMP
```
...
```mermaid
flowchart TB
    OUT[Role outcomes and customer risks] --> DOM[Competency domains]
    DOM --> BEH[Observable behaviors]
    BEH --> COND[Conditions tools and authority]
    COND --> LEVEL[Proficiency levels]
    LEVEL --> EVID[Evidence and assessor]
    EVID --> GAP[Gap and practice plan]
    GAP --> RECHECK[Reassessment and autonomy decision]
```
...
```mermaid
flowchart LR
    SELF[Self-assessment and goals] --> SAMPLE[Work sample scenario or teach-back]
    SAMPLE --> OBS[Assessor observation]
    OBS --> MAP[Competency level and evidence confidence]
    MAP --> PRI[Prioritize by role need consequence and leverage]
    PRI --> PLAN[Study practice shadow feedback and milestone]
    PLAN --> RE[Reassess with changed scenario]
```
...
sequenceDiagram
    ...
    participant R as Mentor
    participant G as Manager or role owner
    participant P as Peers/specialists
    M->>R: Share goals baseline context and questions
    R->>M: Agree contract cadence boundaries and evidence
    G->>M: Confirm role priorities access and authority
    M->>P: Study shadow and practice with approved experts
    M->>R: Bring artifact decision and reflection
    R->>M: Ask questions give behavioral feedback assign practice
    M->>M: Apply and record result
    R->>G: Share agreed progress evidence not private content
    G-->>M: Adjust scope and autonomy
```
...
```mermaid
flowchart LR
    PRE[Pre-brief objective context roles and confidentiality] --> OBS[Observe decisions evidence and language]
    OBS --> PRED[Predict next step and reason]
    PRED --> PART[Contribute bounded artifact or segment]
    PART --> REV[Reverse-shadow full bounded task]
    REV --> DEB[Debrief rubric evidence and alternatives]
    DEB --> RETRY[Changed scenario or next live opportunity]
    RETRY --> AUTH[Autonomy decision]
```
...
```mermaid
flowchart LR
    ASK[Ask for self-assessment and intent] --> OBS[Describe observed behavior/evidence]
    OBS --> IMP[Explain consequence]
    IMP --> STD[Connect to agreed standard/rubric]
    STD --> PRACT[Practice improved behavior]
    PRACT --> COM[Commit to next application]
    COM --> REV[Review evidence and adapt]
```
...
```mermaid
flowchart TB
    CASE[Select representative case] --> CONTEXT[Objective scope roles constraints]
    CONTEXT --> EVID[Evidence timeline and hypotheses]
    EVID --> DEC[Decisions alternatives authority]
    DEC --> ACT[Actions communication and validation]
    ACT --> OUT[Outcome residual and customer acceptance]
    OUT --> QUAL[Rubric strengths and gaps]
    QUAL --> SYS[Knowledge process product or training improvement]
    SYS --> FOLLOW[Owner effectiveness and recurrence review]
```
...
```mermaid
flowchart TB
    Q[Service quality] --> CUST[Customer relevance and impact]
    Q --> TECH[Technical accuracy and evidence]
    Q --> SAFE[Security privacy ethics and authority]
    Q --> PROC[Ownership timeliness and process]
    Q --> COMM[Clarity empathy and expectation]
    Q --> VAL[Validation outcome and residual]
    Q --> KNOW[Documentation learning and reuse]
```
...
```mermaid
sequenceDiagram
    participant O as Work owner
    participant R as Reviewer
    participant C as Calibration group
    participant M as Manager/practice owner
    O->>R: Submit authorized sample and self-review
    R->>R: Apply rubric with evidence
    R->>C: Compare ambiguous ratings and anchor cases
    C-->>R: Calibrate criteria and examples
    R->>O: Give strengths gaps and practice action
    O-->>R: Apply correction or changed-scenario evidence
    R->>M: Report aggregate pattern and systemic action
```
...
```mermaid
flowchart LR
    NEED[Repeated question case or change] --> CAP[Capture candidate learning]
    CAP --> VALID[Validate technical product security and scope]
    VALID --> DESIGN[Define audience task format metadata]
    DESIGN --> REVIEW[Peer/product/privacy review]
    REVIEW --> PUB[Publish in approved repository]
    PUB --> FIND[Search discover and use]
    FIND --> FEED[Feedback outcome and gap]
    FEED --> UPDATE[Update merge or retire]
    UPDATE --> PUB
```
...
```mermaid
flowchart TB
    COP[Community of practice] --> CLINIC[Case clinics]
    COP --> OFFICE[Office hours]
    COP --> DEMO[Pattern demos and teach-backs]
    COP --> CAL[Quality/rubric calibration]
    COP --> KNO[Knowledge review days]
    COP --> TREND[Product/security/data trend discussion]
    CLINIC --> BACK[Knowledge and improvement backlog]
    OFFICE --> BACK
    DEMO --> BACK
    CAL --> BACK
    KNO --> BACK
...
```mermaid
flowchart LR
    ACT[Activity: sessions articles reviews] --> REACH[Reach: intended audience accessed]
    REACH --> LEARN[Learning: demonstrated capability]
    LEARN --> APPLY[Application: behavior in work]
    APPLY --> QUAL[Quality: rubric and acceptance]
    QUAL --> REUSE[Reuse: artifact helps repeated work]
    REUSE --> OUT[Outcome: customer/team effect with limits]
```
...
```mermaid
flowchart LR
    D0[Before/Days 1-10: orient and protect] --> D30[By Day 30: explain and assist]
    D30 --> D60[By Day 60: perform supervised bounded work]
    D60 --> D90[By Day 90: own bounded outcomes and contribute knowledge]
    D90 --> POST[After Day 90: deepen product/account scope and coach selectively]
```
...
sequenceDiagram
    ...
    participant B as Buddy/mentor
    participant M as Manager
    participant C as Customer/account work
    L->>B: Propose bounded task objective evidence and risks
    B->>L: Review scope and intervention triggers
    L->>C: Perform assigned segment under supervision
    B-->>L: Observe and intervene if needed
    L->>B: Self-review artifact decisions and unknowns
    B->>L: Rubric feedback and changed-scenario practice
    B->>M: Provide agreed evidence
    M-->>L: Expand maintain or narrow autonomy
```
...
```mermaid
flowchart TB
    TASK[Defined task and consequence] --> EVID{Knowledge and guided evidence present?}
    EVID -- No --> LEARN[Study and guided practice]
    EVID -- Yes --> REV{Reverse-shadow passed including failure/limits?}
    REV -- No --> COACH[Coach and retry changed scenario]
    REV -- Yes --> SAFE{Security privacy product and authority judgment sound?}
    SAFE -- No --> NARROW[Narrow scope and targeted practice]
    SAFE -- Yes --> CONS{Multiple accepted samples or strong bounded evidence?}
    CONS -- No --> SUP[Continue supervised work]
    CONS -- Yes --> AUTH[Manager grants bounded autonomy and review trigger]
```
...
```mermaid
flowchart TB
    LEARN[Case lesson or repeated question] --> REUSE{Recurring or high-consequence need?}
    REUSE -- No --> CASE[Keep appropriate case note]
    REUSE -- Yes --> EXIST{Current trusted artifact exists?}
    EXIST -- Yes --> UPDATE[Update merge redirect or add example]
    EXIST -- No --> VALID{Technical product privacy and scope validated?}
    VALID -- No --> HOLD[Assign validation; do not publish]
    VALID -- Yes --> DESIGN[Choose audience task format metadata owner]
    DESIGN --> TEST[Test discovery use failure and validation]
    TEST --> PUB[Publish with review/retirement trigger]
```
...
```mermaid
flowchart TB
    CLAIM[Contribution statement] --> ACT{Specific work and scope evidenced?}
    ACT -- No --> REWRITE[State activity or unknown]
    ACT -- Yes --> ACCEPT{Accepted or used by intended owner/audience?}
    ACCEPT -- No --> OUTPUT[Call it a draft/output]
    ACCEPT -- Yes --> EFFECT{Observed effect under stable definition?}
    EFFECT -- No --> USE[State accepted use only]
    EFFECT -- Yes --> ALT{Alternatives/confounders reviewed?}
    ALT -- No --> ASSOC[State association and limit]
    ALT -- Yes --> BOUND[State bounded contribution; avoid sole causation]
```
