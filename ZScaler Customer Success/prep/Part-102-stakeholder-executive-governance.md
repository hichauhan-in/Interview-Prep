...
```mermaid
flowchart LR
    OUT[Customer outcome and risk] --> MAP[Map affected and influential roles]
    MAP --> AUTH[Clarify authority and decision rights]
    AUTH --> NEED[Understand evidence incentives and concerns]
    NEED --> ENG[Choose engagement and communication]
    ENG --> GOV[Set working steering and executive cadence]
    GOV --> DEC[Make record and communicate decisions]
    DEC --> ACT[Execute validate and manage residual]
    ACT --> REMAP[Review changes in influence trust and roles]
    REMAP --> MAP
```
...
```mermaid
flowchart TB
    OUT[Customer outcome] --> OWNS[Owns or accepts]
    OUT --> WORK[Performs workflow or change]
    OUT --> EVID[Provides evidence dependency or expertise]
    OUT --> GOV[Governs risk privacy legal change or commercial]
    OUT --> AFFECT[Affected users services or teams]
    TSM[TSM and account team] --> GUIDE[Guide coordinate support and verify boundaries]
    GUIDE --> OWNS
    GUIDE --> WORK
    GUIDE --> EVID
    GUIDE --> GOV
...
```mermaid
flowchart LR
    ROLE[Role identified] --> LINK{Linked to outcome?}
    LINK -- No --> PARK[Do not over-engage]
    LINK -- Yes --> AUTH[Map authority influence and impact]
    AUTH --> NEED[Map evidence incentive concern and trust]
    NEED --> MODE[Choose forum channel depth and frequency]
    MODE --> ACTION[Assign engagement action and owner]
    ACTION --> REVIEW[Observe decision behavior and remap]
```
...
```mermaid
flowchart TB
    CISO[CISO and security leadership] --> RISK[Risk direction and sponsorship]
    CIO[CIO and IT leadership] --> PORT[Service portfolio and resources]
    SOC[SecOps and VM] --> FLOW[Security workflow and evidence]
    IT[IT network identity data apps] --> EXEC[Technical ownership and change]
    GOV[Compliance privacy legal risk] --> AUTH[Governance conditions]
    PROC[Procurement finance vendor] --> COMM[Commercial authority]
    CHAMP[Champions neutrals detractors] --> ADOPT[Local trust and behavior]
    RISK --> OUT[Customer outcome]
    PORT --> OUT
    FLOW --> OUT
    EXEC --> OUT
    AUTH --> OUT
...
```mermaid
quadrantChart
    title Influence and interest guide engagement, not human value
    x-axis Low interest --> High interest
    y-axis Low influence --> High influence
    quadrant-1 Partner closely
    quadrant-2 Keep satisfied and decision-ready
    quadrant-3 Monitor respectfully
    quadrant-4 Enable and keep informed
    Executive sponsor: [0.75, 0.88]
    Outcome owner: [0.90, 0.78]
    Specialist operator: [0.82, 0.48]
    Adjacent executive: [0.35, 0.72]
```
...
```mermaid
flowchart LR
    REC[Recommendation with evidence] --> INPUT[Required specialist and affected-role input]
    INPUT --> AGREE[Policy-required approval or agreement]
    AGREE --> DECIDE[One explicit decision owner]
    DECIDE --> DO[Responsible performers]
    DO --> VALID[Independent or accountable validation]
    VALID --> RECORD[Decision result and residual recorded]
```
...
```mermaid
flowchart TB
    FACT[One governed fact pattern] --> EXEC[Executive: outcome option decision residual]
    FACT --> PROG[Program: milestone dependency owner forecast]
    FACT --> TECH[Technical: architecture evidence test tradeoff]
    FACT --> OPS[Operator: workflow exception support]
    FACT --> GOV[Governance: purpose data authority control]
    FACT --> COMM[Commercial: verified scope terms and owner]
```
...
```mermaid
flowchart TB
    WORK[Technical working group] --> DEL[Delivery and adoption forum]
    DEL --> STEER[Customer steering committee]
    STEER --> EXEC[Executive or business review]
    EVENT[Event-driven critical escalation] --> WORK
    EVENT --> STEER
    SUPPORT[Support and product lane] --> WORK
    COMM[Commercial lane] --> STEER
    EXEC --> STRAT[Strategic direction and next outcomes]
    STRAT --> DEL
```
...
```mermaid
flowchart TD
    DEC[Decision type and urgency] --> LEVEL{Lowest forum with authority?}
    LEVEL -- Technical --> WORK[Working group]
    LEVEL -- Cross-workstream --> DEL[Delivery forum]
    LEVEL -- Scope risk priority --> STEER[Steering]
    LEVEL -- Strategic investment --> EXEC[Executive review]
    DEC --> URG{Can scheduled cadence wait?}
    URG -- No --> EVENT[Event-driven escalation]
    URG -- Yes --> LEVEL
```
...
sequenceDiagram
    ...
    participant O as Outcome owner
    participant D as Decision roles
    participant W as Workstream owners
    F->>W: Request evidence and changes by cutoff
    W-->>F: Submit milestone risk adoption and decisions
    F->>D: Send concise pre-read with options and asks
    D->>D: Review and prepare questions
    F->>O: Open with outcome and material change
    O->>D: Confirm priority and decision authority
    D->>D: Discuss exception options evidence and tradeoffs
    D-->>F: Record decision owner rationale and conditions
    F-->>W: Publish actions evidence and escalation
```
...
```mermaid
flowchart LR
    CONCERN[Observe or hear concern] --> LISTEN[Restate without rebuttal]
    LISTEN --> CLASS[Classify evidence incentive impact and authority]
    CLASS --> TEST[Agree safe discriminating test or decision]
    TEST --> OPTION[Offer repair alternative guardrail defer or stop]
    OPTION --> DECIDE[Right owner decides]
    DECIDE --> FOLLOW[Report result and residual]
```
...
```mermaid
flowchart TD
    CONFLICT[Stakeholder conflict] --> TYPE{Goal authority evidence resource scope or history?}
    TYPE --> FACTS[Separate facts assumptions interests and positions]
    FACTS --> SHARED[Restate shared customer outcome and constraints]
    SHARED --> OPTIONS[Generate options and tradeoffs]
    OPTIONS --> OWNER{Decision right clear?}
    OWNER -- No --> ESC[Escalate authority gap]
    OWNER -- Yes --> DEC[Owner decides with evidence]
    DEC --> RECORD[Record rationale conditions residual and follow-up]
```
...
```mermaid
flowchart LR
    TRIG[Material trigger] --> TRIAGE[Validate scope impact and evidence]
    TRIAGE --> PATH[Select customer support product commercial or executive path]
    PATH --> ASK[State exact decision or assistance needed]
    ASK --> CAD[Set update cadence and communication owner]
    CAD --> RESULT[Record decision mitigation and validation]
    RESULT --> CLOSE[Close or transfer residual with learning]
```
...
```mermaid
flowchart LR
    FORUM[Forum receives decision-ready input] --> DEC[Authorized decision]
    DEC --> ACTION[Owned actions and conditions]
    ACTION --> VALID[Evidence and outcome validation]
    VALID --> RESID[Residual and next trigger]
    RESID --> LEARN[Governance improvement]
    LEARN --> FORUM
```
...
```mermaid
flowchart TD
    INFO[Proposed stakeholder communication] --> NEED{Recipient needs it for role or decision?}
    NEED -- No --> OMIT[Do not share]
    NEED -- Yes --> AUTH{Authorized channel purpose and audience?}
    AUTH -- No --> RESOLVE[Resolve governance]
    AUTH -- Yes --> MIN{Can content be minimized or aggregated?}
    MIN -- Yes --> REDACT[Use bounded form]
    MIN -- No --> SHARE[Share approved detail]
    REDACT --> RECORD[Record material decision not unnecessary personal data]
    SHARE --> RECORD
```
...
```mermaid
flowchart TD
    FAIL[Governance or relationship symptom] --> DEC{Decision right clear?}
    DEC -- No --> MAP[Repair authority and RACI]
    DEC -- Yes --> EVID{Decision-ready evidence available?}
    EVID -- No --> BUILD[Assign evidence owner and deadline]
    EVID -- Yes --> FORUM{Right forum cadence and quorum?}
    FORUM -- No --> REDESIGN[Change charter participants or trigger]
    FORUM -- Yes --> TRUST{Concern incentive or history unresolved?}
    TRUST -- Yes --> REPAIR[Listen test acknowledge and follow through]
    TRUST -- No --> EXECUTE[Decide record act validate]
```
...
```mermaid
flowchart TD
    D[Decision needed] --> TYPE{Technical customer risk commercial product or support?}
    TYPE --> POLICY[Identify governing policy contract and scope]
    POLICY --> AUTH{One accountable authority known?}
    AUTH -- No --> ESC[Escalate authority gap before execution]
    AUTH -- Yes --> INPUT[Collect necessary expert and affected-role input]
    INPUT --> DECIDE[Authorized role decides]
    DECIDE --> RECORD[Record rationale conditions owner and residual]
```
...
```mermaid
flowchart TD
    S[Stakeholder role] --> LINK{Outcome authority evidence impact or dependency link?}
    LINK -- No --> MON[Monitor or inform only if needed]
    LINK -- Yes --> AUTH{High authority or influence?}
    AUTH -- Yes --> PARTNER[Decision-ready partnership]
    AUTH -- No --> IMP{Highly affected or operationally essential?}
    IMP -- Yes --> INVOLVE[Involve in design test and feedback]
    IMP -- No --> INFORM[Tailored information]
    PARTNER --> POS[Adapt to concern position and trust]
    INVOLVE --> POS
    INFORM --> POS
```
...
```mermaid
flowchart TD
    I[Issue or conflict] --> MAT{Material impact security privacy scope or critical path?}
    MAT -- No --> LOCAL[Resolve at lowest authorized level]
    MAT -- Yes --> OWN{Current forum has authority and time?}
    OWN -- Yes --> DEC[Make and record decision]
    OWN -- No --> PACK[Prepare bounded evidence impact options and ask]
    PACK --> PATH[Use customer support product commercial or executive path]
    PATH --> CAD[Set cadence until transfer or closure]
```
