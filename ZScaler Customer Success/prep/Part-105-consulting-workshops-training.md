...
```mermaid
flowchart LR
    NEED[Customer need or decision] --> AUD[Analyze audience authority and context]
    AUD --> OBJ[Contract observable objectives]
    OBJ --> MODE[Choose discovery design demo practice or training]
    MODE --> PREP[Prepare evidence environment roles and safeguards]
    PREP --> DELIVER[Facilitate participation and decisions]
    DELIVER --> CHECK[Observe exercise teach-back and decision evidence]
    CHECK --> FOLLOW[Publish read-back actions resources and gaps]
    FOLLOW --> APPLY[Measure later application and improve]
```
...
```mermaid
flowchart TB
    PEOPLE[Invited and affected people] --> ROLE[Role and job to perform]
    PEOPLE --> KNOW[Prior knowledge and misconceptions]
    PEOPLE --> STAKE[Goals risks incentives and concerns]
    PEOPLE --> AUTH[Authority influence and dependencies]
    PEOPLE --> ACCESS[Language time zone accessibility and tools]
    ROLE --> DESIGN[Depth examples activity and output]
    KNOW --> DESIGN
    STAKE --> DESIGN
    AUTH --> DESIGN
    ACCESS --> DESIGN
...
```mermaid
flowchart LR
    EXEC[Executive: outcome risk decision] --> COMMON[Shared fact pattern]
    ARCH[Architect: boundaries options tradeoffs] --> COMMON
    OPS[Operator: steps evidence failure recovery] --> COMMON
    GOV[Governance: authority privacy audit] --> COMMON
    COMMON --> LAYER[Layered narrative and artifacts]
```
...
```mermaid
flowchart LR
    NEED[Work need] --> BEHAVIOR[Observable behavior]
    BEHAVIOR --> CONDITION[Conditions and resources]
    CONDITION --> CRITERIA[Accuracy safety and quality criteria]
    CRITERIA --> ACTIVITY[Demo exercise discussion or design method]
    ACTIVITY --> EVIDENCE[Artifact observation decision or teach-back]
    EVIDENCE --> FOLLOW[Later application check]
```
...
```mermaid
flowchart TB
    START[Customer request] --> KNOWN{Current state goals constraints and authority sufficiently known?}
    KNOWN -- No --> DISC[Discovery session]
    DISC --> READ[Read-back facts assumptions unknowns]
    READ --> READY{Evidence sufficient for bounded design?}
    READY -- No --> TEST[Collect evidence or narrow scope]
    TEST --> DISC
    READY -- Yes --> DESIGN[Design session]
    KNOWN -- Yes --> DESIGN
    DESIGN --> OPTIONS[Requirements options tradeoffs]
    OPTIONS --> DECIDE{Authorized decision?}
    DECIDE -- No --> RECOMMEND[Recommendation and open decision]
    DECIDE -- Yes --> RECORD[Decision implementation and validation plan]
```
...
sequenceDiagram
    ...
    participant E as Evidence owners
    participant D as Decision owner
    S->>F: State need audience and intended use
    F->>P: Validate roles knowledge constraints and access
    F->>E: Request approved pre-work and samples
    F->>S: Contract charter objectives scope and outputs
    F->>F: Build run-of-show materials exercises and contingencies
    F->>P: Open contract and activate prior knowledge
    P->>F: Contribute evidence questions designs and practice
    F->>D: Read back options decisions unknowns and risks
    D-->>F: Decide or assign evidence and decision route
    F->>P: Publish artifacts actions resources and evaluation
    P-->>F: Demonstrate later application or report blockers
```
...
```mermaid
flowchart LR
    OPEN[Open contract and safety] --> ACTIVATE[Activate prior knowledge]
    ACTIVATE --> MODEL[Explain or co-create model]
    MODEL --> OBSERVE[Demonstrate bounded behavior]
    OBSERVE --> PRACTICE[Guided then independent practice]
    PRACTICE --> TEACH[Teach-back or decision]
    TEACH --> CLOSE[Read-back actions resources evaluation]
```
...
```mermaid
flowchart LR
    ACTOR[User analyst admin or service] --> ID[Identity and context]
    ID --> ENTRY[Device network API or data entry]
    ENTRY --> CONTROL[Verified policy or control point]
    CONTROL --> APP[Application service or analytical layer]
    APP --> DATA[Data source model or store]
    DATA --> FLOW[Workflow decision ticket or action]
    FLOW --> OWNER[Human owner and authority]
    CONTROL -. logs and evidence .-> OBS[Observability]
    APP -. errors and health .-> OBS
    DATA -. quality lineage freshness .-> OBS
    FLOW -. status audit outcome .-> OBS
```
...
sequenceDiagram
    ...
    participant E as Approved environment
    participant O as Evidence point
    F->>L: State claim setup scope and limitation
    F->>L: Ask learner to predict behavior
    F->>E: Perform one bounded action
    E->>O: Produce state event or error
    O-->>F: Return observable evidence
    F->>L: Compare expected and observed
    L->>F: Explain mechanism and next check
    F->>L: Record result unknowns and applicability boundary
```
...
```mermaid
flowchart LR
    MODEL[Facilitator models method] --> GUIDED[Group performs with prompts]
    GUIDED --> PAIR[Pairs perform with job aid]
    PAIR --> SOLO[Individual or role performs]
    SOLO --> VARIATION[New scenario or failure]
    VARIATION --> TEACH[Teach-back reasoning]
    TEACH --> FEEDBACK[Rubric feedback and retry]
    FEEDBACK --> TRANSFER[Approved work application]
```
...
```mermaid
flowchart TB
    OBJ[Objective and participants] --> MODE{Which mode supports access evidence and participation?}
    MODE --> VIRT[Virtual]
    MODE --> SITE[On site]
    MODE --> HYB[Hybrid]
    VIRT --> VP[Producer captions digital board breakouts backup dial-in]
    SITE --> SP[Room layout physical/digital capture visitor and device controls]
    HYB --> HP[Remote-first artifacts individual audio explicit turn taking]
    VP --> READY[Readiness rehearsal]
    SP --> READY
    HP --> READY
```
...
```mermaid
flowchart TB
    DIS[Disagreement appears] --> SAFE{Personal or unsafe behavior?}
    SAFE -- Yes --> STOP[Restate ground rule pause or escalate]
    SAFE -- No --> CLAIM[Write exact competing claims]
    CLAIM --> CLASS[Separate facts assumptions values and preferences]
    CLASS --> EVID{Evidence available now?}
    EVID -- Yes --> TEST[Compare source scope time and authority]
    EVID -- No --> OWNER[Assign evidence owner and decision route]
    TEST --> RESOLVE{Resolved for current objective?}
    RESOLVE -- Yes --> READ[Read back conclusion and implication]
    RESOLVE -- No --> OWNER
```
...
```mermaid
flowchart LR
    REACT[Reaction: relevant usable accessible] --> LEARN[Learning: can explain or perform]
    LEARN --> APPLY[Application: uses method in approved work]
    APPLY --> OUT[Outcome: workflow or risk measure changes]
    OUT --> REVIEW[Review attribution context and unintended effects]
    REVIEW --> IMPROVE[Improve objective content practice and support]
    IMPROVE --> REACT
```
...
```mermaid
flowchart LR
    SESSION[Session evidence] --> CLEAN[Correct classify minimize and version]
    CLEAN --> READBACK[Decisions assumptions unknowns and actions]
    READBACK --> AIDS[Diagram job aid recording or resources as approved]
    AIDS --> OWNER[Owner due-date basis support and escalation route]
    OWNER --> APPLY[Application checkpoint]
    APPLY --> REVIEW[Case review metric review or coaching]
    REVIEW --> UPDATE[Improve artifact training or design]
```
...
```mermaid
flowchart TB
    NEED[Need stated] --> INCIDENT{Active supported-product incident?}
    INCIDENT -- Yes --> SUPPORT[Use Support and escalation process; add education only if useful]
    INCIDENT -- No --> STATE{Current state and need sufficiently known?}
    STATE -- No --> DISC[Discovery workshop]
    STATE -- Yes --> CHOICE{Future option or architecture decision?}
    CHOICE -- Yes --> DESIGN[Design workshop]
    CHOICE -- No --> SHOW{Need bounded behavior illustrated?}
    SHOW -- Yes --> DEMO[Verified demonstration plus observation]
    SHOW -- No --> PERFORM{Need participant performance?}
    PERFORM -- Yes --> TRAIN[Training with exercise teach-back and application]
    PERFORM -- No --> BRIEF[Briefing job aid or asynchronous resource]
```
...
```mermaid
flowchart TB
    CLAIM[Exact demo claim] --> VERIFY{Current docs entitlement setup and expected behavior verified?}
    VERIFY -- No --> PREP[Verify or change to conceptual explanation]
    VERIFY -- Yes --> DATA{Approved data identity and environment?}
    DATA -- No --> SYN[Synthetic or sanitized environment]
    DATA -- Yes --> RISK{Authorized reversible and monitored?}
    SYN --> RISK
    RISK -- No --> REDESIGN[Use read-only recording or offline evidence]
    RISK -- Yes --> BACKUP{Failure and follow-up path ready?}
    BACKUP -- No --> PREP
    BACKUP -- Yes --> RUN[Run bounded demo and record limits]
```
...
```mermaid
flowchart TB
    OBJ[Observable objective] --> EVID{Participant produced valid evidence?}
    EVID -- No --> COACH[Diagnose gap coach and retry]
    EVID -- Yes --> SAFE{Safety limits and failure handling demonstrated?}
    SAFE -- No --> PRACTICE[Additional scenario and recovery practice]
    SAFE -- Yes --> SCOPE{Evidence covers required scope?}
    SCOPE -- No --> PARTIAL[Record bounded readiness and remaining plan]
    SCOPE -- Yes --> APPLY[Approve appropriate next application by customer process]
    APPLY --> LATER[Observe later use and outcome separately]
```
