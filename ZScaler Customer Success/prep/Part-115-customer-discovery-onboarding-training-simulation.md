...
```mermaid
flowchart LR
    PREP[Prepare hypotheses and boundaries] --> DISC[Discover outcomes current state constraints]
    DISC --> READ[Read back facts assumptions unknowns]
    READ --> ARCH[Map architecture data and workflows]
    ARCH --> OUT[Contract measurable outcomes]
    OUT --> PLAN[Build phased success plan]
    PLAN --> ONB[Onboard roles sources and operating cadence]
    ONB --> WORK[Facilitate synthetic technical workshop]
    WORK --> ADOPT[Teach back and adoption evidence]
    ADOPT --> FOLLOW[Decisions actions risks follow-up]
    FOLLOW --> REVIEW[Health value and next discovery]
```
...
```mermaid
flowchart TD
    SAFE[Part 111 safety honesty contract] --> PACK[Local synthetic account pack]
    PACK --> ROLE[Choose participant roles or self-role-play]
    ROLE --> TIME[Schedule simulation blocks]
    TIME --> TOOLS[Editor spreadsheet slides or Mermaid]
    TOOLS --> LABEL[Visible SYNTHETIC NMH LAB labels]
    LABEL --> READY{No real customer data or product login}
    READY -- No --> STOP[Replace with synthetic fixtures]
    READY -- Yes --> START[Begin simulation]
```
...
```mermaid
flowchart TD
    WHY[Business and security outcomes] --> WHO[Stakeholders jobs authority]
    WHO --> TODAY[Current workflows architecture and pain]
    TODAY --> DATA[Sources grain quality privacy]
    DATA --> DEC[Priority decision and treatment]
    DEC --> OPS[Ownership SLA exception validation]
    OPS --> MEASURE[Baseline target guardrail acceptance]
    MEASURE --> CON[Constraints dependencies change readiness]
    CON --> READ[Read back facts assumptions unknowns decisions]
```
...
```mermaid
flowchart TB
    CISO[CISO sponsor] --> STEER[Monthly steering decision forum]
    VM[VM Lead] --> OPS[Weekly exposure operations]
    SOC[SOC Manager] --> OPS
    DATA[Security Data Architect] --> DQ[Twice weekly data quality workstream]
    CMDB[CMDB Owner] --> DQ
    EDR[Endpoint Lead] --> DQ
    APP[Application Owners] --> REM[Remediation workstream]
    ITSM[ITSM Owner] --> REM
    PRIV[Privacy Officer] --> GATE[Data use and retention gate]
    GATE --> DQ
    OPS --> STEER
    DQ --> OPS
    REM --> OPS
...
```mermaid
flowchart LR
    subgraph CUSTOMER[Fictional NMH environment]
        CMDB[CMDB inventory and ownership]
        EDR[Endpoint and control evidence]
        SCAN[Vulnerability scanner findings]
        IAM[Identity and privilege context]
        APP[Application and business registry]
        ITSM[Ticketing and changes]
        SOC[Threat and incident context]
    end
    subgraph LEARNING[Vendor-neutral target learning model]
        ING[Ingest and source contracts]
        Q[Quality freshness reconciliation]
        ENT[Canonical entities and provenance]
        CTX[Relationships and business context]
        PRI[Transparent prioritization]
        FLOW[Owner exception validation workflow]
        VIEW[Technical and executive views]
    end
    CUSTOMER --> ING --> Q --> ENT --> CTX --> PRI --> FLOW --> VIEW
```
...
```mermaid
sequenceDiagram
    participant S as Scanner source
    participant A as VM analyst
    participant C as CMDB lookup
    participant T as Ticketing
    participant O as Technical owner
    participant V as Validation source
    S->>A: Emit severity-ranked finding
    A->>C: Search asset owner and application
    C-->>A: Sometimes duplicate stale or missing context
    A->>A: Manually research threat and control context
    A->>T: Create or update ticket
    T->>O: Assign work
    O-->>T: Close after reported change
    T-->>A: Closure status
    A->>V: Periodic reconciliation or rescan
    V-->>A: Sometimes finding remains open
```
...
```mermaid
flowchart LR
    CAP[Verified data and workflow capability] --> BEH[Repeated correct operator behavior]
    BEH --> EFFECT[Operational effect]
    EFFECT --> OUT[Customer accepted outcome]
    OUT --> VALUE[Qualified value narrative]
    DATA[Quality and source health] --> CAP
    GOV[Owners authority and cadence] --> BEH
    GUARD[Safety privacy availability guardrails] --> OUT
```
...
```mermaid
flowchart TD
    P0[Phase 0 Governance and outcomes] --> G0{Scope authority privacy accepted}
    G0 -- Pass --> P1[Phase 1 Source and architecture readiness]
    P1 --> G1{Contracts quality and access evidence accepted}
    G1 -- Pass --> P2[Phase 2 Canonical data and priority pilot]
    P2 --> G2{Match score confidence and calibration pass}
    G2 -- Pass --> P3[Phase 3 Workflow and validation pilot]
    P3 --> G3{Owners tickets exceptions validation operate}
    G3 -- Pass --> P4[Phase 4 Training and bounded adoption]
    P4 --> G4{Teach back and sampled application pass}
    G4 -- Pass --> P5[Phase 5 Review value hypothesis and scale decision]
    G0 -- Fail --> HOLD[Hold narrow redesign]
    G1 -- Fail --> HOLD
    G2 -- Fail --> HOLD
    G3 -- Fail --> HOLD
    G4 -- Fail --> HOLD
...
```mermaid
flowchart LR
    FIND[Raw synthetic finding] --> CHECK[Check source health and grain]
    CHECK --> ENTITY[Resolve asset and provenance]
    ENTITY --> CONTEXT[Add business threat control context]
    CONTEXT --> PRIOR[Explain score confidence and priority]
    PRIOR --> OWNER[Choose owner treatment and due basis]
    OWNER --> VALID[Define validation residual exception]
    VALID --> TELL[Explain technical and executive narrative]
```
...
```mermaid
flowchart TD
    LISTEN[Listen without interrupting] --> VALID[Validate impact or concern]
    VALID --> CLARIFY[Clarify exact proposition and stakes]
    CLARIFY --> CLASS[Classify fact assumption requirement preference]
    CLASS --> EVID[Review evidence and unknowns]
    EVID --> BOUND[State product security commercial authority boundary]
    BOUND --> OPT[Offer options test owner and next checkpoint]
    OPT --> CONF[Confirm whether concern is addressed]
```
...
```mermaid
flowchart LR
    NOTES[Raw simulation notes] --> CLASS[Classify facts assumptions unknowns]
    CLASS --> CORR[Correct architecture and outcome map]
    CORR --> DEC[Decision and rationale register]
    DEC --> ACT[Actions owners due bases validation]
    ACT --> PLAN[Updated success plan and risks]
    PLAN --> RES[Resources and support routes]
    RES --> READ[Customer read-back and correction window]
```
...
```mermaid
flowchart TD
    S1[1 Prepare account hypotheses] --> S2[2 Contract discovery]
    S2 --> S3[3 Interview role cards]
    S3 --> S4[4 Read back current state]
    S4 --> S5[5 Build stakeholder architecture maps]
    S5 --> S6[6 Define outcomes metrics]
    S6 --> S7[7 Build onboarding success plan]
    S7 --> S8[8 Facilitate technical workshop]
    S8 --> S9[9 Run objections]
    S9 --> S10[10 Assess teach back]
    S10 --> S11[11 Issue follow up]
    S11 --> S12[12 Changed case validation and release]
```
...
```mermaid
flowchart LR
    BRIEF[Hypothesis account brief] --> DISC[Discovery charter notes read back]
    DISC --> MAP[Stakeholder architecture workflow maps]
    MAP --> OUT[Outcome and metric contracts]
    OUT --> PLAN[Onboarding success and risk plans]
    PLAN --> WORK[Workshop exercises teach back]
    WORK --> OBJ[Objection and decision ledger]
    OBJ --> FOLLOW[Follow up package]
    FOLLOW --> RUB[Changed cases rubric reflection]
```
...
```mermaid
flowchart TD
    STALL[Engagement simulation stalls] --> GOAL{Outcome and decision agreed}
    GOAL -- No --> REDISC[Return to sponsor and workflow discovery]
    GOAL -- Yes --> FACT{Facts assumptions unknowns separated}
    FACT -- No --> READ[Run corrective read back]
    FACT -- Yes --> OWNER{Authority and ownership present}
    OWNER -- No --> MAP[Repair stakeholder RACI and escalation]
    OWNER -- Yes --> DEP{Dependencies and gates explicit}
    DEP -- No --> PLAN[Rebuild evidence gated plan]
    DEP -- Yes --> CAP{Participant capability demonstrated}
    CAP -- No --> PRACT[Practice teach back and retry]
    CAP -- Yes --> FOLLOW{Decisions and actions accepted}
    FOLLOW -- No --> CLOSE[Repair follow up and checkpoint]
    FOLLOW -- Yes --> REVIEW[Proceed to bounded review]
```
...
```mermaid
flowchart TD
    INV[Inventory notes maps slides recordings] --> REAL{Any real person customer tenant or account data}
    REAL -- Yes --> STOP[Stop exclude and follow applicable policy]
    REAL -- No --> LABEL{Every artifact visibly synthetic}
    LABEL -- No --> FIX[Correct labels]
    LABEL -- Yes --> META[Review author paths comments hidden data]
    META --> SELECT[Select sanitized evidence set]
    SELECT --> DELETE[Delete exact temporary drafts]
    DELETE --> VERIFY[Open release and test links]
...
```mermaid
flowchart TD
    SAFE[Safety privacy honesty gates] --> PREP[Hypothesis based preparation]
    PREP --> DISC[Discovery and read back]
    DISC --> MAP[Stakeholder architecture outcome]
    MAP --> PLAN[Onboarding success plan]
    PLAN --> WORK[Workshop teach back]
    WORK --> OBJ[Objections and boundaries]
    OBJ --> FOL[Follow up and changed cases]
    FOL --> PASS{At least 80 and no critical defect}
    PASS -- Yes --> READY[Portfolio-ready synthetic simulation]
    PASS -- No --> REWORK[Correct rehearse rescore]
```
