...
```mermaid
flowchart LR
    DISC[Qualified discovery and current state] --> HAND[Account and delivery handoff]
    HAND --> KICK[Kickoff and operating agreement]
    KICK --> PRE[Prerequisites and readiness gates]
    PRE --> FOUND[Foundation and safe configuration]
    FOUND --> PILOT[Pilot one bounded workflow]
    PILOT --> ADOPT[Enable repeated correct use]
    ADOPT --> VALUE[Validate technical and customer outcome]
    VALUE --> SCALE[Scale govern and improve]
```
...
```mermaid
flowchart TD
    DISC[Discovery baseline] --> VERIFY[Revalidate material facts]
    VERIFY --> HANDOFF[Resolve commitment and ownership gaps]
    HANDOFF --> OUTCOMES[Confirm outcomes and success evidence]
    OUTCOMES --> PHASES[Design phases and gates]
    PHASES --> WORK[Assign tasks dependencies and cadence]
    WORK --> RISK[Manage RAID and changes]
    RISK --> ACCEPT[Accept milestones with customer evidence]
```
...
```mermaid
sequenceDiagram
    participant S as Sales or account owner
    participant T as TSM
    participant PS as Services or implementation owner
    participant SUP as Support
    participant C as Customer sponsor and technical owner
    S->>T: Share documented goals scope commitments and boundaries
    PS->>T: Confirm delivery ownership assumptions and dependencies
    SUP->>T: Share authorized relevant case state and risks
    T->>T: Separate facts assumptions unknowns and conflicts
    T->>C: Read back purpose scope roles prerequisites and decisions
    C-->>T: Correct approve reshape or defer kickoff plan
```
...
```mermaid
flowchart LR
    WHY[Why and outcome] --> WHAT[Scope and success]
    WHAT --> WHO[Roles and authority]
    WHO --> HOW[Phases gates and evidence]
    HOW --> NEED[Prerequisites dependencies and risk]
    NEED --> WORK[Cadence documentation and escalation]
    WORK --> NOW[First owned actions]
```
...
```mermaid
flowchart TB
    SP[Customer sponsor] --> PRIORITY[Outcome priority and escalation]
    OWNER[Customer outcome owner] --> ACCEPT[Acceptance and residual risk]
    PROG[Program owner] --> PLAN[Integrated plan and cadence]
    DOM[Domain owners] --> EXEC[Prerequisites changes and validation]
    TSM[TSM] --> ORCH[Success orchestration and guidance]
    SERV[Services] --> DELIV[Contracted delivery]
    SUP[Support] --> CASE[Incident diagnosis]
    SALES[Sales] --> COMM[Commercial scope]
    PROD[Product and Engineering] --> TRUTH[Product feedback and defect path]
```
...
stateDiagram-v2
    [*] --> Proposed
    Proposed --> EvidencePending
    EvidencePending --> Ready: acceptance evidence passes
    EvidencePending --> Blocked: condition fails or owner missing
    Blocked --> EvidencePending: remediation completed
    Ready --> GateReview
    GateReview --> Accepted: accountable owner approves
    GateReview --> Conditional: bounded exception with expiry
    Conditional --> Accepted: condition resolved
    Conditional --> Blocked: condition breaches stop rule
    Accepted --> Stale: material change or validity expires
...
```mermaid
flowchart TB
    OUT[Outcome and success contract] --> ROAD[Phased roadmap and milestones]
    ROAD --> WORK[Workstreams tasks and prerequisites]
    WORK --> EVID[Acceptance evidence]
    WORK --> RAID[Risk assumption issue dependency]
    WORK --> ADOPT[Enablement and adoption]
    EVID --> HEALTH[Health and value review]
    RAID --> HEALTH
    ADOPT --> HEALTH
    HEALTH --> DEC[Decisions changes and next phase]
```
...
```mermaid
flowchart LR
    P0[Align] --> G0{Outcome and scope gate}
    G0 --> P1[Prepare]
    P1 --> G1{Readiness gate}
    G1 --> P2[Validate foundation]
    P2 --> G2{Technical acceptance}
    G2 --> P3[Pilot workflow]
    P3 --> G3{Safety quality usability}
    G3 --> P4[Operationalize and adopt]
    P4 --> G4{Repeated use and supportability}
    G4 --> P5[Validate outcome]
    P5 --> G5{Customer value decision}
    G5 --> P6[Scale or improve]
```
...
stateDiagram-v2
    [*] --> Planned
    Planned --> InProgress
    InProgress --> EvidenceReady
    EvidenceReady --> Review
    Review --> Accepted
    Review --> Conditional
    Review --> Rejected
    Conditional --> InProgress: condition remains
    Rejected --> InProgress: repair or reshape
    Accepted --> Reopened: evidence invalidated or scope changed
    Reopened --> InProgress
```
...
```mermaid
flowchart TD
    GOV[Purpose privacy and security approval] --> ACCESS[Scoped identity and access]
    ARCH[Architecture and source authority] --> ACCESS
    ACCESS --> CONN[Connectivity and source validation]
    CONN --> QUAL[Data quality and reconciliation]
    QUAL --> PILOT[Pilot workflow]
    ROLE[Owner approval and user capacity] --> PILOT
    CHANGE[Change window and rollback] --> PILOT
    PILOT --> TRAIN[Enablement with real workflow]
    TRAIN --> ADOPT[Repeated use]
    ADOPT --> VALUE[Outcome validation]
```
...
```mermaid
timeline
    title Time-to-value clocks must use explicit events
    Mobilize : documented handoff
             : kickoff agreement
    Ready    : prerequisites accepted
             : pilot authorized
    Technical: bounded workflow passes
             : operations accepted
    Adopt    : eligible roles use workflow repeatedly
    Value    : customer accepts outcome evidence
```
...
```mermaid
flowchart TD
    CAND[Candidate early win] --> REL{Relevant to customer outcome?}
    REL -- No --> DROP[Do not call it value]
    REL -- Yes --> SAFE{Bounded and low consequence?}
    SAFE -- No --> CONTROL[Add gate approval rollback and test]
    SAFE -- Yes --> EVID{Baseline and acceptance evidence feasible?}
    EVID -- No --> LEARN[Use as learning task not outcome claim]
    EVID -- Yes --> REUSE{Builds reusable foundation?}
    REUSE -- No --> RECONSIDER[Avoid cosmetic dead end]
    REUSE -- Yes --> PILOT[Deliver validate and communicate honestly]
```
...
```mermaid
flowchart LR
    ELIG[Eligible roles and cases] --> ACCESS[Secure access]
    ACCESS --> KNOW[Knowledge and purpose]
    KNOW --> PRACTICE[Guided practice]
    PRACTICE --> USE[Repeated correct use]
    USE --> RESULT[Validated workflow effect]
    RESULT --> FEED[Feedback support and improvement]
    FEED --> KNOW
```
...
```mermaid
flowchart TB
    TECH[Technical working sessions] --> WEEK[Weekly integrated delivery]
    ADOPT[Adoption and champion review] --> WEEK
    WEEK --> STEER[Monthly steering decisions]
    STEER --> PHASE[Phase or quarterly outcome review]
    EVENT[Event-driven risk or escalation] --> WEEK
    EVENT --> STEER
    PHASE --> PLAN[Rebaseline next outcome]
```
...
```mermaid
flowchart LR
    IDENT[Identify risk assumption issue dependency] --> CLASS[Classify and scope]
    CLASS --> OWN[Assign accountable owner]
    OWN --> RESP[Prevent mitigate contingency or validate]
    RESP --> TRIG[Monitor evidence and trigger]
    TRIG --> DEC[Escalate decide or close]
    DEC --> RESID[Record residual and learning]
```
...
```mermaid
flowchart TD
    CHANGE[Proposed onboarding change] --> AUTH{Authorized owner and scope?}
    AUTH -- No --> STOP[Do not proceed]
    AUTH -- Yes --> MIN{Minimum data access and effect?}
    MIN -- No --> REDUCE[Reduce scope or privilege]
    MIN -- Yes --> TEST{Safe test rollback and monitoring ready?}
    TEST -- No --> PREP[Complete controls]
    TEST -- Yes --> EXEC[Execute in approved window]
    EXEC --> READ[Read back technical and business state]
    READ --> ACCEPT[Accept rollback or remediate]
```
...
```mermaid
flowchart TD
    SLIP[Milestone slips or health degrades] --> OUT{Outcome and acceptance still valid?}
    OUT -- No --> REBASE[Re-discover and rebaseline]
    OUT -- Yes --> DEP{First failed or unknown dependency?}
    DEP -- Unknown --> TRACE[Trace critical path and evidence]
    DEP -- Found --> OWN{Owner and authority available?}
    OWN -- No --> ESC[Escalate decision-right gap]
    OWN -- Yes --> OPT[Develop repair narrow parallel defer or stop options]
    OPT --> DEC[Customer/account owners decide]
    DEC --> PLAN[Update plan forecast residual and communication]
```
...
```mermaid
flowchart TD
    START[Proposed pilot] --> OUT{Outcome scope and acceptor defined?}
    OUT -- No --> ALIGN[Return to discovery]
    OUT -- Yes --> PRE{Critical commercial technical data and governance prerequisites pass?}
    PRE -- No --> SEQ[Sequence readiness work]
    PRE -- Yes --> SAFE{Test monitoring rollback and support ready?}
    SAFE -- No --> CONTROL[Complete safety and operations]
    SAFE -- Yes --> USERS{Eligible users and owner capacity available?}
    USERS -- No --> PLAN[Replan cohort or timing]
    USERS -- Yes --> GO[Authorize bounded pilot]
```
...
```mermaid
flowchart TD
    ITEM[Candidate output] --> REL{Customer-relevant decision or uncertainty?}
    REL -- No --> ACT[Call it activity, not value]
    REL -- Yes --> PROOF{Baseline and acceptance evidence?}
    PROOF -- No --> LEARN[Call it learning and build proof]
    PROOF -- Yes --> GUARD{Security privacy quality and operations acceptable?}
    GUARD -- No --> FIX[Repair guardrails]
    GUARD -- Yes --> ACCEPT{Customer owner accepts bounded effect?}
    ACCEPT -- No --> REVIEW[Revise or reject]
    ACCEPT -- Yes --> WIN[Communicate bounded early win]
```
...
```mermaid
flowchart TD
    MISS[Forecast or milestone miss] --> CAUSE{First failed dependency known?}
    CAUSE -- No --> TRACE[Trace evidence and critical path]
    CAUSE -- Yes --> IMP{Outcome scope security or commercial impact?}
    IMP -- Material --> STEER[Escalate options to decision owner]
    IMP -- Local --> REPAIR[Repair with owner and acceptance]
    STEER --> CHOICE[Repair narrow sequence defer or stop]
    REPAIR --> UPDATE[Update forecast RAID and evidence]
    CHOICE --> UPDATE
```
