...
Arti must not describe this exercise as Deloitte client work, a production SOC deployment, ingestion of Arti/employer logs, a real incident, or live automated response. A `datatable` query demonstrates KQL reasoning, not connector health. A paper DCR demonstrates architecture, not data collection. A Logic Apps diagram is not a deployed playbook.
