...
```mermaid
flowchart LR
    Q[Behavioral question] --> P[Past example]
    P --> C[Competency signal]
    C --> F[Interviewer feedback]
    F --> D[Hiring decision]
```
...
```mermaid
flowchart LR
    S[Situation<br/>What was happening?] --> T[Task<br/>What was my responsibility?]
    T --> A[Action<br/>What exactly did I do?]
    A --> R[Result<br/>What changed?]
    R --> L[Learning<br/>What would I repeat or improve?]
```
...
sequenceDiagram
    ...
    participant I1 as Interviewer 1
    participant I2 as Interviewer 2
    participant AA as AA Interviewer
    participant D as Debrief / Decision

    R->>Candidate: Recruiter screen
    HM->>Candidate: Hiring manager round
    I1->>Candidate: Technical / behavioral round
    I2->>Candidate: Cross-functional / problem-solving round
    AA->>Candidate: Values / culture / growth-mindset round
    I1->>D: Written feedback
    I2->>D: Written feedback
    AA->>D: Written feedback
    HM->>D: Hiring recommendation
    D->>Candidate: Final decision / next steps
```
...
```mermaid
mindmap
  root((Microsoft values in stories))
    Growth mindset
      Feedback
      Learning
...
```mermaid
flowchart TD
    Q[Interview question] --> C{Which competency?}
    C -->|Data / impact| S1[Story 1 or 5]
    C -->|Pressure / customer| S2[Story 2 or 10]
    C -->|Process / ownership| S3[Story 3]
    C -->|Influence / ambiguity| S4[Story 4 or 9]
    C -->|Mentoring / care| S6[Story 6 or 11]
    C -->|Learning / growth| S8[Story 8 or 12]
```
...
```mermaid
flowchart LR
    A[Support systems / surveys / case events] --> B[Ingest]
    B --> C[Clean and transform]
    C --> D[Curated model / semantic layer]
    D --> E[Dashboard / report / self-serve]
    E --> F[Business decision or operational action]
```
