...
```mermaid
flowchart LR
    Align[Align role access quality and risk] --> Learn[Learn product domain systems and policies]
    Learn --> Observe[Shadow and narrate evidence decisions]
    Observe --> Practice[Perform labs and reviewed simulations]
    Practice --> Own[Own bounded work with review]
    Own --> Measure[Measure quality customer outcomes and coaching themes]
    Measure --> Broaden{Manager agrees evidence supports broader scope}
    Broaden -->|Not yet| Practice
    Broaden -->|Yes| Contribute[Broader ownership knowledge feedback and improvement]
    Contribute --> Plan[Agree next-quarter development plan]
```
...
flowchart TD
    ...
    L1 --> Product[Product and knowledge quality]
    L1 --> CSM[Customer Success]
    L1 --> Guard[Security privacy legal and incident owners]
    Manager --> Gate[Scope quality and readiness gates]
    Deep --> Evidence[Deeper finding or fix]
    Product --> Guidance[Behavior decision or reusable guidance]
    CSM --> Context[Adoption and stakeholder context]
    Guard --> Rules[Authority handling and disclosure rules]
    Evidence --> L1
    Guidance --> L1
    Context --> L1
    Rules --> L1
...
```mermaid
flowchart TD
    Intake[Approved bounded case] --> Scope[Impact expected actual scope and safe evidence]
    Scope --> Hypotheses[Rank hypotheses and select discriminating check]
    Hypotheses --> Boundary{Within current access skill runbook and risk boundary}
    Boundary -->|No| Escalate[Escalate with explicit ask and retain customer ownership]
    Boundary -->|Yes| Test[Run supported test or guidance]
    Test --> Result{Expected state verified}
    Result -->|No| Escalate
    Result -->|Yes| Close[Confirm customer outcome document and close]
    Escalate --> Partner[Collaborate and update customer]
    Partner --> Result
    Close --> Review[Quality review coaching and reusable learning]
```
...
```mermaid
flowchart LR
    Read[Read current source] --> Explain[Explain and predict]
    Explain --> Observe[Observe approved work]
    Observe --> Practice[Practice safely]
    Practice --> Perform[Perform assigned bounded work]
    Perform --> Review[Review against local rubric]
    Review --> Correct[Correct and retest]
    Correct --> Reuse{Stable and reusable}
    Reuse -->|No| Read
    Reuse -->|Yes| Share[Share through approved knowledge or improvement path]
    Share --> Read
```
...
sequenceDiagram
    ...
    A->>M: Bring evidence outcomes errors blockers and proposed focus
    M->>R: Request calibration on selected work when needed
    R-->>A: Give specific quality product and boundary feedback
    A->>A: Correct artifact or behavior and retest
    A-->>M: Show changed evidence not only completion
    M->>M: Decide maintain narrow or broaden scope
    M-->>A: Agree next milestone target guardrail and checkpoint
    A-->>C: Apply only approved behavior in assigned work
```
