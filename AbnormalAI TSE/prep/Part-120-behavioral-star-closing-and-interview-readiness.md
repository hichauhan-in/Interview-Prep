...
```mermaid
flowchart LR
    Facts["Verified background facts"] --> Stories["Candidate-completed true stories"]
    Sources["Dated official sources"] --> Fit["Bounded fit answers"]
    Bank["Part 119 prompts"] --> Mock["Uncued spoken mocks"]
    Stories --> Mock
    Fit --> Mock
    Mock --> Score["Evidence-based score and corrections"]
    Score --> Ready{"Readiness gates passed repeatedly?"}
    Ready -->|No| Repair["Practice weakest observable skill"]
    Repair --> Mock
    Ready -->|Yes| Calibrated["Calibrated interview readiness"]
```
...
```mermaid
flowchart TD
    Draft["Draft experience sentence"] --> Master{"Explicitly supported by master or verified CV?"}
    Master -->|Yes| Detail{"Real sanitized story details personally verified?"}
    Detail -->|Yes| Production["Use direct production-transfer wording"]
    Detail -->|No| Placeholder["Keep brackets; do not perform the story"]
    Master -->|No| Run{"Safe lab actually performed with artifact?"}
    Run -->|Yes| LabTier["Use performed-lab wording and limits"]
    Run -->|No| Docs{"Current official source studied?"}
    Docs -->|Yes| Learned["Use learned-architecture wording"]
    Docs -->|No| Gap["State no direct experience and ramp plan"]
```
...
```mermaid
flowchart TD
    Prompt["Behavioral prompt"] --> Competency["Name the competency being tested"]
    Competency --> Inventory{"Real sanitized event in story inventory?"}
    Inventory -->|No| Honest["Say no strong example is prepared; use a truthful adjacent example or framework"]
    Inventory -->|Yes| Match{"Does the event actually demonstrate this competency?"}
    Match -->|No| Other["Choose another event; do not bend facts"]
    Match -->|Yes| Verify["Check situation task actions result and reflection"]
    Verify --> Risk{"Any invented metric confidential detail or false attribution?"}
    Risk -->|Yes| Remove["Remove or generalize safely without changing truth"]
    Risk -->|No| Speak["Deliver naturally and invite follow-up"]
    Remove --> Speak
```
...
```mermaid
flowchart TD
    FitQ["Fit or motivation question"] --> Direct["Answer the question in one sentence"]
    Direct --> Evidence["Give one or two verified background anchors"]
    Evidence --> Company{"Company-specific claim needed?"}
    Company -->|Yes| Source["Attribute dated official source and its limit"]
    Company -->|No| Role["Connect to role outcome"]
    Source --> Role
    Role --> Gap{"Material gap affects credibility?"}
    Gap -->|Yes| Name["Name gap bridge and measurable ramp"]
    Gap -->|No| Stop["Stop and invite follow-up"]
    Name --> Stop
```
...
```mermaid
flowchart LR
    Apply["Application and recruiter contact"] --> RecruiterRound["Possible recruiter conversation"]
    RecruiterRound --> HM["Possible hiring-manager round"]
    HM --> Tech["Possible technical and troubleshooting panel"]
    Tech --> Cross["Possible behavioral or cross-functional conversations"]
    Cross --> Final["Possible final alignment and decision"]
    RecruiterRound -. "sequence may differ" .-> Tech
    HM -. "rounds may combine" .-> Cross
...
```mermaid
flowchart TD
    Question["Question arrives"] --> Type{"Past behavior fit or technical scenario?"}
    Type -->|Past behavior| Real{"Real relevant sanitized event available?"}
    Real -->|Yes| STARPath["STAR plus reflection and role relevance"]
    Real -->|No| Adjacent{"Truthful adjacent event available?"}
    Adjacent -->|Yes| Limit["State difference then use adjacent evidence"]
    Adjacent -->|No| Framework["State no strong example; explain framework without inventing history"]
    Type -->|Fit| FitPath["Motivation plus evidence plus role connection plus gap"]
    Type -->|Technical scenario| TechPath["Clarify scope reason aloud test safely validate"]
    STARPath --> Check["Check attribution confidentiality and claim tier"]
    Limit --> Check
    Framework --> Check
    FitPath --> Check
    TechPath --> Check
    Check --> Close["Answer directly then stop"]
```
...
```mermaid
flowchart TD
    RedFlag["Unsupported or sensitive statement appears"] --> Stop["Stop adding detail"]
    Stop --> Type{"Fabrication privacy product claim or ordinary wording issue?"}
    Type -->|Fabrication| Retract["Correct explicitly and remove claim"]
    Type -->|Privacy| Contain["End capture restrict/delete artifact follow approved reporting if needed"]
    Type -->|Product claim| Bound["Attribute source state unknown and validation path"]
    Type -->|Wording| Clarify["Separate I/we observation/inference and result"]
    Retract --> Review["Return to evidence ledger and retest"]
    Contain --> Review
    Bound --> Review
    Clarify --> Review
```
...
```mermaid
flowchart LR
    Select["Select unseen mixed prompts"] --> Record["State confidence and answer uncued"]
    Record --> Probe["Reviewer or prompt card adds follow-up"]
    Probe --> Close["Give concise result limit and next step"]
    Close --> Score["Score preserved first attempt"]
    Score --> Diagnose["Name one technical one communication and one honesty gap"]
    Diagnose --> Repair["Review linked Part and rewrite anchors"]
    Repair --> Delay["Wait based on score"]
    Delay --> Variant["Retry changed prompt without script"]
    Variant --> Score
```
...
stateDiagram-v2
    ...
    Practicing --> ReadyEvidence: All readiness gates repeatedly pass
    ReadyEvidence --> Practicing: Material gap source change or score decay
```
...
```mermaid
flowchart LR
    Evening["Night before"] --> Facts["Verify stories boundaries and two questions"]
    Facts --> Setup["Check time route link device audio and backup"]
    Setup --> Rest["Stop cramming and rest"]
    Rest --> Morning["Interview day: brief anchors only"]
    Morning --> Join["Join or arrive early without opening sensitive systems"]
    Join --> Listen["Listen clarify pause and answer"]
    Listen --> Close["Ask questions concise close and record private follow-up notes"]
```
