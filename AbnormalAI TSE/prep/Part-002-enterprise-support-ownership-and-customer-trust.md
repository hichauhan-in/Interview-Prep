...
```mermaid
flowchart LR
    Signal[Customer reports an unwanted condition] --> Frame[Frame outcome impact and scope]
    Frame --> Plan[Choose next discriminating action]
    Plan --> Act{Who can perform it safely}
    Act -->|L1| L1Work[L1 performs supported work]
    Act -->|Customer| CustWork[Customer performs authorized action]
    Act -->|Specialist| SpecWork[Senior support or Engineering acts]
    Act -->|Decision owner| Decide[Product CSM or SOC decides]
    L1Work --> Observe[Record result and update evidence]
    CustWork --> Observe
    SpecWork --> Observe
    Decide --> Observe
    Observe --> Outcome{Customer outcome verified}
    Outcome -->|No| Plan
    Outcome -->|Yes| Learn[Close confirm and capture learning]
```
...
```mermaid
flowchart TB
    Concern[Concern: external events business deadlines unknown defects and decisions outside authority]
    Influence[Influence: specialist attention customer choices Product clarity and cross-team alignment]
    Control[Control: case framing notes evidence quality updates asks commitments and escalation timing]
    Control --> Influence
    Influence --> Concern
```
...
stateDiagram-v2
    [*] --> Acknowledged
    Acknowledged --> Scoped: impact expected and observed captured
    Scoped --> Testing: hypothesis and safe test selected
    Testing --> Scoped: result changes scope or hypothesis
    Testing --> AwaitingCustomer: authorized customer evidence or action needed
    Testing --> Collaborating: specialist access or decision needed
    AwaitingCustomer --> Testing: requested result received
    Collaborating --> Testing: specialist result received
    Testing --> Validation: corrective path available
    Validation --> Testing: original symptom persists
    Validation --> Resolved: customer outcome confirmed
    Resolved --> KnowledgeCapture
    KnowledgeCapture --> Closed
    Closed --> [*]
```
...
sequenceDiagram
    ...
    C->>L: Reports impact and observed behavior
    L-->>C: Reflects impact and commits next checkpoint
    L->>L: Scopes evidence and tests supported hypotheses
    L->>S: Warm escalation with explicit technical ask
    S-->>L: Accepts and requests internal telemetry review
    L->>E: Supplies reproducible evidence and identifiers
    L-->>C: Sends proactive update before silence grows
    E-->>L: Returns finding or corrective path
    L->>L: Validates result against original scenario
    L-->>C: Explains result limitations and asks for confirmation
    C-->>L: Confirms outcome
```
...
```mermaid
flowchart TD
    Core[One evidence core: impact facts unknowns action owner time] --> Admin[Customer admin: steps settings and validation]
    Core --> SOC[Customer SOC: scope indicators risk and response boundary]
    Core --> Exec[Executive: business effect risk decisions and forecast limits]
    Core --> Senior[Senior support: tests evidence and escalation criteria]
    Core --> Eng[Engineering: repro expected actual IDs and explicit ask]
    Core --> Product[Product: user job pattern impact workaround and evidence]
    Core --> CSM[CSM: relationship adoption milestones and stakeholder context]
```
...
sequenceDiagram
    ...
    L->>S: Context impact evidence attempts explicit ask
    S-->>L: Accepts ownership of advanced diagnosis
    L-->>C: Names collaboration and keeps customer cadence
    S->>E: Requests internal telemetry with request IDs
    E-->>S: Finding and validation condition
    S-->>L: Returns finding limits and next action
    L->>P: Requests intended-behavior clarification if needed
    P-->>L: Records decision without roadmap promise
    L-->>C: Explains outcome and requests validation
```
...
```mermaid
flowchart TD
    Start[Case appears stalled or trust is declining] --> State{Can you state outcome impact evidence and next action}
    State -->|No| Reframe[Reframe case and build next-action ledger]
    State -->|Yes| Owner{Does next action have an accepted owner and time}
    Reframe --> Owner
    Owner -->|No| Assign[Name actor purpose evidence and checkpoint]
    Owner -->|Yes| Boundary{Is action within authority skill and safety boundary}
    Assign --> Boundary
    Boundary -->|No| Warm[Warm handoff with explicit ask and acceptance]
    Boundary -->|Yes| Test[Run lowest-risk discriminating action]
    Warm --> Update{Is customer update due before result}
    Test --> Update
    Update -->|Yes| Communicate[Send impact evidence uncertainty action owner and time]
    Update -->|No| Observe[Record result]
    Communicate --> Observe
    Observe --> Outcome{Original customer outcome verified}
    Outcome -->|No| Learn[Update hypothesis state and next action]
    Learn --> State
    Outcome -->|Yes| Confirm[Confirm closure limitations monitoring and follow-ups]
    Confirm --> Capture[Capture verified reusable knowledge or product signal]
```
