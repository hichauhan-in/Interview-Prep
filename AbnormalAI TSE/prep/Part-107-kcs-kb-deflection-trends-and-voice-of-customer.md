...
```mermaid
flowchart LR
    Need["Customer need or support question"] --> Search["Search in the flow of work"]
    Search --> Found{"Applicable knowledge found?"}
    Found -->|Yes| Reuse["Reuse with scope and evidence checks"]
    Found -->|No| Solve["Investigate safely and capture learning"]
    Reuse --> Outcome{"Outcome validated?"}
    Solve --> Outcome
    Outcome -->|Yes| Improve["Improve or create knowledge candidate"]
    Outcome -->|No| Escalate["Continue diagnosis or escalate"]
    Improve --> Review["Review audience accuracy safety and ownership"]
    Review --> Publish["Publish internally or externally if authorized"]
    Publish --> Observe["Observe searches reuse feedback outcomes and demand"]
    Observe --> Improve
```
...
```mermaid
flowchart TB
    subgraph Solve["Solve Loop in an interaction"]
        Context["Capture customer context"] --> Structure["Structure for reuse"]
        Structure --> Reuse["Search and reuse"]
        Reuse --> Improve["Improve from use"]
        Improve --> Context
    end
    subgraph Evolve["Evolve Loop across interactions"]
        Health["Assess content health"] --> Process["Improve process and findability"]
        Process --> Performance["Assess outcomes with guardrails"]
        Performance --> Leadership["Coach communicate and govern"]
        Leadership --> Health
    end
    Improve --> Health
    Performance --> Reuse
...
```mermaid
flowchart LR
    Learning["Validated reusable learning"] --> Audience{"Who needs it?"}
    Audience -->|Authorized support staff| Internal["Internal article candidate"]
    Audience -->|Customer or public| External["External article candidate"]
    Audience -->|Both| Pair["Related internal and external pair"]
    Internal --> InternalReview["Technical privacy permission and owner review"]
    External --> ExternalReview["Technical safety Product legal brand accessibility and publication review as required"]
    Pair --> InternalReview
    Pair --> ExternalReview
    InternalReview --> InternalPublish{"Approved for stated internal scope?"}
    ExternalReview --> ExternalPublish{"Approved for stated external scope?"}
    InternalPublish -->|No| DraftI["Keep controlled draft or reject"]
    InternalPublish -->|Yes| UseI["Internal reuse and feedback"]
    ExternalPublish -->|No| DraftE["Keep controlled draft or reject"]
    ExternalPublish -->|Yes| UseE["External self-service and feedback"]
```
...
sequenceDiagram
    ...
    participant Search as Search experience
    participant Index as Permitted index
    participant Article as Governed article
    participant Feedback as Knowledge feedback route
    User->>Search: Query in customer language
    Search->>Index: Match terms taxonomy access and rank
    Index-->>Search: Permitted candidate results
    Search-->>User: Ranked results or no-result state
    alt Relevant article found
        User->>Article: Open and check scope
        Article-->>User: Safe procedure and validation
        User->>Feedback: Outcome/helpfulness or correction signal
    else Wrong or no result
        User->>Search: Refine query or choose assisted support
        Search->>Feedback: Record safe query/session signals
    end
```
...
```mermaid
flowchart TD
    Start["New case learning search signal or article feedback"] --> Sensitive{"Customer data secret proprietary content security risk or restricted evidence?"}
    Sensitive -->|Yes| Secure["Stop copying or publishing; preserve minimum reference and use authorized route"]
    Sensitive -->|No| Existing{"Existing article found for the same audience outcome and scope?"}
    Existing -->|Yes| Applicable{"Current accurate safe permitted and applicable?"}
    Existing -->|No| Reusable{"Learning validated and likely reusable?"}
    Applicable -->|Yes| Reuse["Reuse and link with applicability and outcome evidence"]
    Applicable -->|No| Failure{"Why is it unusable?"}
    Failure -->|Stale wrong unsafe| Restrict["Stop reuse; flag restrict correct supersede or retire"]
    Failure -->|Hard to find or understand| Improve["Improve title synonyms structure tags or navigation within authority"]
    Failure -->|Different intent or scope| Split["Create or route a separate candidate if justified"]
    Reusable -->|No one-off or uncertain| CaseOnly["Keep controlled case learning; continue evidence collection"]
    Reusable -->|Yes| Audience{"Internal external or both?"}
    Audience --> Internal["Draft internal article candidate"]
    Audience --> External["Draft external article candidate"]
    Audience --> Both["Draft linked audience-specific pair"]
    Internal --> Review["Required technical privacy security owner review"]
    External --> ReviewExt["Required technical safety Product legal brand accessibility publication review"]
    Both --> Review
    Both --> ReviewExt
    Reuse --> Pattern{"Repeated friction or product need supported by evidence?"}
    Improve --> Pattern
    Split --> Pattern
    CaseOnly --> Pattern
    Pattern -->|No| Monitor["Monitor governed signals and review triggers"]
    Pattern -->|Yes| Brief["Create VOC or Product evidence brief with uncertainty"]
    Brief --> Decision["Request owner decision; do not promise roadmap or cause"]
    Review --> PublishI{"Approved?"}
    ReviewExt --> PublishE{"Approved?"}
    PublishI -->|Yes| InternalLive["Publish to authorized internal audience"]
    PublishI -->|No| Controlled["Revise reject or keep controlled"]
    PublishE -->|Yes| ExternalLive["Publish to approved external audience"]
    PublishE -->|No| Controlled
```
...
```mermaid
flowchart LR
    Demand["Eligible customer need"] --> Exposure["Self-service opportunity shown or sought"]
    Exposure --> Discovery["Relevant content discovered"]
    Discovery --> Applicability["User recognizes applicable scope"]
    Applicability --> Action["Safe action or answer used"]
    Action --> Success["Intended task outcome succeeds"]
    Success --> NoContact["No avoidable assisted contact in defined window"]
    Exposure --> NoFind["No result or wrong result"]
    Discovery --> Wrong["Content wrong stale unsafe or inaccessible"]
    Action --> Failure["Task fails or user needs assistance"]
    NoFind --> Assisted["Assisted support or abandonment"]
    Wrong --> Assisted
    Failure --> Assisted
    NoContact --> Guardrails["Check repeat contact escalation satisfaction safety and hidden demand"]
    Assisted --> Guardrails
```
...
```mermaid
flowchart TD
    Scope["Define eligible population window and question"] --> Dictionary["Freeze tag dictionary and evidence thresholds"]
    Dictionary --> Quality["Measure completeness agreement defaults and schema changes"]
    Quality --> Normalize["Normalize comparable records without erasing uncertainty"]
    Normalize --> Segment["Segment by product area journey stage cohort and outcome"]
    Segment --> Rank["Rank counts impact or effort and compute cumulative share"]
    Rank --> Investigate["Inspect examples and competing explanations"]
    Investigate --> Decision{"Knowledge Product process incident or data-quality action?"}
    Decision --> Brief["Write trend and evidence brief"]
    Brief --> Monitor["Track intervention plus quality guardrails"]
    Monitor --> Scope
```
...
```mermaid
sequenceDiagram
    participant Support as Support and Knowledge
    participant Data as Approved evidence sources
    participant Analyst as VOC analyst or owner
    participant Product as Product and Engineering
    participant Customer as Customer-facing owner
    Support->>Data: Contribute minimized tagged patterns and article feedback
    Data-->>Analyst: Aggregated cases searches outcomes surveys and limits
    Analyst->>Analyst: Validate definitions segments examples and competing explanations
    Analyst->>Product: Evidence brief with problem outcome impact uncertainty and explicit ask
    Product-->>Analyst: Classify decide request more evidence or decline/defer
    Analyst->>Support: Decision rationale scope and next evidence need
    Support->>Customer: Approved factual update without roadmap promise
```
...
```mermaid
flowchart TD
    Symptom["Fictional export complete with zero rows"] --> Safety{"Security privacy data-loss broad-impact or permission risk?"}
    Safety -->|Yes| Escalate["Stop and use authorized escalation route"]
    Safety -->|No| Eligible{"Any authored eligible source records in selected window?"}
    Eligible -->|No| Explain["Explain empty eligible set; validate with harmless synthetic record only if authorized"]
    Eligible -->|Yes| Window{"Time zone boundary and window include records?"}
    Window -->|No| Correct["Correct fictional selection in paper model and re-evaluate"]
    Window -->|Yes| Visibility{"Authorized role can view eligible records through approved path?"}
    Visibility -->|No| Permission["Do not broaden access; route permission question to owner"]
    Visibility -->|Yes| Compare{"Known-safe synthetic comparison differs?"}
    Compare -->|No both empty| Scope["Check shared prerequisite and evidence ceiling"]
    Compare -->|Yes| Difference["Document one discriminating input difference"]
    Scope --> Unknown["Keep mechanism unknown and escalate with minimum packet"]
    Difference --> Unknown
    Explain --> Validate{"Intended task outcome validated?"}
    Correct --> Validate
    Validate -->|Yes| Reuse["Record outcome and article applicability feedback"]
    Validate -->|No| Unknown
```
...
```mermaid
flowchart TD
    Charter["Approve question eligibility privacy and definitions"] --> Baseline["Measure comparable baseline and data quality"]
    Baseline --> Design{"Causal design ethical feasible and authorized?"}
    Design -->|Yes| Experiment["Predeclare assignment outcomes guardrails and stopping rules"]
    Design -->|No| Observe["Use observational design and label confounding limits"]
    Experiment --> Run["Run only after publication and analytics approvals"]
    Observe --> Run
    Run --> Quality["Audit missing data tagging identity matching and instrumentation"]
    Quality --> Outcome["Estimate task success and contact behavior by segment"]
    Outcome --> Guard["Check repeat contact effort escalation safety accessibility and abandonment"]
    Guard --> Interpret{"Evidence supports change?"}
    Interpret -->|No or harmful| Rollback["Stop revise restore route and document"]
    Interpret -->|Uncertain| Learn["Collect approved discriminating evidence"]
    Interpret -->|Yes within limits| Adopt["Adopt for stated scope and continue monitoring"]
```
...
```mermaid
flowchart TD
    Signal["Knowledge trend metric or feedback concern"] --> Safety{"Customer data secret security privacy legal accessibility or destructive risk?"}
    Safety -->|Yes| Stop["Stop processing copying publishing or rollout; use authorized incident/privacy/security/legal route"]
    Safety -->|No| Truth{"Fabricated metric causal overclaim or materially wrong content?"}
    Truth -->|Yes| Correct["Preserve audit trail restrict claim notify owner and correct through governance"]
    Truth -->|No| Scope{"Broad customer impact suspected?"}
    Scope -->|Yes| Incident["Assess incident/problem route and maintain customer ownership"]
    Scope -->|No| Ownership{"Correct owner and authority known?"}
    Ownership -->|No| Manager["Escalate exact decision and keep current ownership visible"]
    Ownership -->|Yes| Classify{"Primary need?"}
    Classify -->|Content accuracy or publication| Knowledge["Knowledge technical and publication owners"]
    Classify -->|Search taxonomy or analytics| Ops["Knowledge/Search/Support Operations and privacy owner"]
    Classify -->|Product usability or behavior| Product["Product or Engineering with evidence brief"]
    Classify -->|Metric/incentive gaming| Governance["Leadership analytics quality ethics or compliance route"]
    Knowledge --> Track["Record owner decision scope and review trigger"]
    Ops --> Track
    Product --> Track
    Governance --> Track
```
...
```mermaid
flowchart TD
    Charter["Write exact local synthetic no-account charter"] --> Define["Define labels taxonomy outcomes and evidence tiers"]
    Define --> Cases["Author forty fictional cases across four weeks"]
    Cases --> Audit["Audit tags unknowns totals and classification ambiguity"]
    Audit --> Pareto["Calculate rank share and cumulative share"]
    Pareto --> Article["Draft internal and external article candidates"]
    Article --> Decisions["Run reuse findability and safety cases through decision tree"]
    Decisions --> Brief["Write trend VOC and Product evidence brief"]
    Brief --> Plan["Write deflection measurement plan and guardrails"]
    Plan --> Gaming["Reject gaming and model content/tag failures"]
    Gaming --> Validate{"All deterministic privacy and honesty gates pass?"}
    Validate -->|No cycle below three| Repair["Record failure and repair only local fiction"]
    Repair --> Validate
    Validate -->|No after cycle three| Incomplete["Keep incomplete and request human review"]
    Validate -->|Yes after actual work| Complete["Mark local synthetic lab completed not transferred"]
```
