...
```mermaid
flowchart LR
    S[S — Situation<br/>set the scene] --> T[T — Task<br/>your goal/challenge]
    T --> A[A — Action<br/>what YOU did]
    A --> R[R — Result<br/>the outcome, quantified]
```
...
```mermaid
flowchart TD
    C[CSM Competencies to evidence] --> C1[Customer focus / empathy]
    C --> C2[Technical problem-solving]
    C --> C3[Handling a difficult customer]
    C --> C4[Cross-team coordination / influence]
    C --> C5[Proactivity / preventing a problem]
    C --> C6[Managing competing priorities]
    C --> C7[Driving adoption / change]
```
...
```mermaid
flowchart TD
    Q[Your questions] --> R[About the Role]
    Q --> T[About the Team/Success]
    Q --> C[About the Customer/Product]
    Q --> G[About Growth]
```
