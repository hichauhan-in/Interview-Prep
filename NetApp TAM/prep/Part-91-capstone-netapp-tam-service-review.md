...
```mermaid
flowchart LR
    DISC[Discovery and objectives] --> DATA[Sources, cutoff and privacy]
    DATA --> RECON[Install-base/data-quality reconciliation]
    RECON --> ANALYZE[Technical/account analysis]
    ANALYZE --> RISK[Risk and priorities]
    RISK --> REC[Recommendations/options]
    REC --> REVIEW[Service-review meeting]
    REVIEW --> ACTION[Decisions/actions]
    ACTION --> PROOF[Follow-up/outcome/residual risk]
```

```mermaid
flowchart TD
    LIVE{Authorized live account work?} -->|No| SYN[Use complete synthetic capstone]
    LIVE -->|Yes| ACCESS{Approved sources/roles/distribution?}
    ACCESS -->|No| STOP[Stop/request authorization]
    ACCESS -->|Yes| LEAD{Lead TAM/SME review ready?}
    LEAD -->|No| DRAFT[Draft only; no customer claim]
    LEAD -->|Yes| CUSTOMER[Authorized customer review]
```
...
```mermaid
flowchart TB
    BUSINESS[Research deadlines/compliance] --> SERVICES[Five critical services]
    SERVICES --> PLATFORM[VMware/Kubernetes/Windows/Linux]
    PLATFORM --> NETWORK[Ethernet/FC/DNS/identity]
    NETWORK --> STORAGE[Two ONTAP clusters and cloud file service concept]
    STORAGE --> PROTECT[Snapshots/SnapMirror/backup/DR]
    TELE[Telemetry/supportability/lifecycle] --> STORAGE
```
...
```mermaid
flowchart LR
    ASK[Discovery question] --> EVID[Evidence/source owner]
    EVID --> ASSUME[Confirmed fact, assumption or unknown]
    ASSUME --> IMPACT[Why it changes analysis/priority]
    IMPACT --> FOLLOW[Owner and due date]
```
...
```mermaid
flowchart TB
    USERS[Researchers] --> NAS[Research NAS service]
    APPS[Database apps] --> SAN[Database SAN service]
    VMS[vSphere analytics VMs] --> NAS
    K8S[Kubernetes catalog] --> NAS
    NAS --> C1[ONTAP cluster nrc-c1]
    SAN --> C2[ONTAP cluster nrc-c2]
    C1 --> DR1[Replication destination]
    C2 --> DR2[Replication destination]
    C1 --> CLOUD[Hybrid cloud archive concept]
    ID[AD/DNS/NTP] --> USERS
    ID --> APPS
    ID --> VMS
    ID --> K8S
...
```mermaid
flowchart LR
    SITEA[Site A power/network] --> C1[Cluster 1]
    SITEA --> C2[Cluster 2]
    FAB[Shared core network] --> C1
    FAB --> C2
    ID[Shared DNS/AD] --> ALL[All services]
    DRSITE[DR site] --> DEST[Destinations]
    WAN[Inter-site WAN] --> DEST
```
...
```mermaid
flowchart LR
    EXPECT[Expected source/population] --> RECEIVE[Received rows/files]
    RECEIVE --> QA[Freshness/schema/units/duplicates/nulls]
    QA --> JOIN[Stable-ID reconciliation]
    JOIN --> COVER[Coverage and exceptions]
    COVER --> USE[Allowed analytical uses]
```
...
```mermaid
flowchart LR
    CMDB[CMDB IDs/effective dates] --> X[Identity crosswalk]
    ONTAP[Observed cluster/node UUIDs] --> X
    TELE[Telemetry identities] --> X
    SUPPORT[Support/install-base equivalent] --> X
    X --> GOLD[Current golden record]
    X --> EX[Retired/missing/stale exceptions]
```
...
```mermaid
flowchart TD
    CLUSTER[Cluster/node/HA health] --> BASE[Health baseline]
    NETWORK[LIF/port/path/events] --> BASE
    STORAGE[Local tier/volume/disk/capacity] --> BASE
    PROTOCOL[NFS/SMB/iSCSI/VMware/K8s] --> BASE
    PROTECT[Snapshots/replication/backup] --> BASE
    SECURITY[Access/audit/advisories] --> BASE
    TELE[Telemetry freshness/coverage] --> BASE
```
...
```mermaid
flowchart LR
    OBS[Observation] --> DEF[Definition/scope/time]
    DEF --> BASELINE[Expected/control]
    BASELINE --> FIND[Finding/confidence]
    FIND --> RISK[Customer objective/trigger/consequence]
```
...
```mermaid
flowchart TB
    APP[Application p50/p95/p99] --> CORR[Common VM/device/host/UTC]
    GUEST[Guest queue/I/O wait] --> CORR
    ESX[ESXi device/path/datastore] --> CORR
    FAB[Network/fabric] --> CORR
    ONTAP[Exact LUN/volume counters/events] --> CORR
    CORR --> HYP[Path-stack hypothesis supported]
```
...
```mermaid
flowchart LR
    BASE[Current physical use] --> TREND[Validated historical growth]
    CHANGE[Snapshot/change/retention] --> TREND
    PROJECT[Known onboarding/projects] --> SCEN[Scenario forecast]
    TREND --> SCEN
    SCEN --> LEAD[Threshold and procurement/change lead time]
    LEAD --> ACTION[Latest safe start]
```

```mermaid
flowchart TD
    CAP[Capacity risk] --> UNITS[Raw/usable/physical/logical/effective units]
    UNITS --> DATAQ[Freshness/outliers/seasonality]
    DATAQ --> SCEN[Base/high/onboarding scenarios]
    SCEN --> OPT[Retention, placement, efficiency, expansion options]
    OPT --> VALID[Outcome/headroom/residual risk]
```
...
```mermaid
flowchart LR
    APP[Service RPO/RTO/dependencies] --> LOCAL[Local snapshots]
    LOCAL --> REP[Replication destination]
    REP --> BACKUP[Independent backup/cyber controls]
    BACKUP --> REST[Restore/DR test]
    REST --> PROOF[App integrity + achieved RPO/RTO]
```
...
```mermaid
flowchart LR
    CONFIG[Reconciled exact configuration] --> IMT[IMT recipe/notes]
    CONFIG --> HWU[HWU facts/limits]
    CONFIG --> LIFE[Release/platform lifecycle]
    CONFIG --> ADV[Advisory applicability]
    CONFIG --> BUG[Authorized bug applicability]
    IMT --> UPG[Upgrade/action options]
    HWU --> UPG
    LIFE --> UPG
    ADV --> UPG
    BUG --> UPG
...
```mermaid
flowchart TD
    CAND[Bug/advisory candidate] --> EXACT[Product/release/feature/path]
    EXACT --> TRIG[Trigger/signature]
    TRIG --> EVID[Customer evidence]
    EVID --> CLASS[Applicable/not/unknown]
    CLASS --> OPT[Mitigation/fix/monitor/current source]
```
...
```mermaid
flowchart LR
    CASES[Case/incident records] --> CLASS[Normalize service/symptom/cause/status]
    CLASS --> RECUR[Recurring pattern and change correlation]
    RECUR --> QUALITY[Evidence/escalation/communication quality]
    QUALITY --> PREVENT[Preventative action/knowledge/change control]
    PREVENT --> MEASURE[Recurrence and experience outcome]
```
...
```mermaid
flowchart LR
    FIND[Finding] --> SCORE[Transparent dimensions]
    SCORE --> CONF[Confidence/data quality]
    CONF --> CONTEXT[Customer criticality/dependencies]
    CONTEXT --> PRIOR[Prioritized risk]
    PRIOR --> REVIEW[Lead TAM/SME/customer context]
```
...
```mermaid
flowchart LR
    STATUS[Status quo and consequence] --> OPTIONS[Options/tradeoffs]
    OPTIONS --> REC[Preferred recommendation]
    REC --> OWNER[Owner/date/dependencies]
    OWNER --> CHANGE[Authorized execution]
    CHANGE --> EFF[Completion + effectiveness]
    EFF --> RES[Residual risk]
```
