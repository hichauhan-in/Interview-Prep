...
```mermaid
flowchart LR
    SIGNAL[Customer signal or evidence] --> MODE{What kind of work is needed?}
    MODE -->|Failure or request exists| REACTIVE[Reactive response]
    MODE -->|Context suggests action| PROACTIVE[Proactive analysis]
    MODE -->|Known exposure can be reduced| PREVENT[Preventative action]
    MODE -->|Model estimates future state| PREDICT[Predictive forecast]
    REACTIVE --> LEARN[Capture learning]
    PROACTIVE --> DECIDE[Customer decision]
    PREVENT --> DECIDE
    PREDICT --> VALIDATE[Validate assumptions and uncertainty]
    VALIDATE --> DECIDE
    LEARN --> PROACTIVE
```
...
```mermaid
flowchart TB
    TRUST[Willingness to rely under uncertainty]
    T[Transparency]
    R[Reliability]
    A[Accuracy and ability]
    C[Customer empathy]
    E[Ethics]
    T --> TRUST
    R --> TRUST
    ...
    C --> TRUST
    E --> TRUST
    DEP[Dependencies and context] -.shape perception.-> TRUST
    HISTORY[Repeated experience] -.compounds.-> TRUST
```
...
stateDiagram-v2
    [*] --> Unknown
    Unknown --> Testing: First interactions
    Testing --> Earned: Accurate advice and kept commitments
    Earned --> Strengthened: Repeated candor and useful outcomes
    Testing --> Damaged: Surprise or unreliable behavior
    Earned --> Damaged: Material miss, concealment, or overpromise
    Strengthened --> Damaged: Serious breach
    Damaged --> RepairAttempt: Acknowledge and investigate
    RepairAttempt --> Rebuilding: Corrective actions and proof
    Rebuilding --> Earned: Repeated verified reliability
    RepairAttempt --> Lost: Denial, blame, or repeated failure
    Lost --> [*]
```
...
sequenceDiagram
    ...
    participant CO as Customer owner
    participant EX as Executive sponsor

    TA->>LT: Validate evidence, impact, uncertainty, and exact ask
    LT->>CO: State the material fact early and plainly
    LT->>CO: Acknowledge impact and prior expectation
    TA->>CO: Separate known, unknown, assumption, and dependency
    TA->>CO: Present options, tradeoffs, owner, and next checkpoint
    CO-->>LT: Challenge evidence or choose an option
    LT->>EX: Escalate decision only if authority or material risk requires it
    TA-->>CO: Send decision record and tracked commitments
```
...
```mermaid
flowchart LR
    SIGNAL[Signal, blocker, or disagreement] --> TRIAGE[Verify impact, scope, owner, and current path]
    TRIAGE --> NEED{Why escalate?}
    NEED -->|Expertise| SME[Add specialist]
    NEED -->|Authority| SPONSOR[Add decision owner or sponsor]
    NEED -->|Capacity| MGMT[Add management attention or resources]
    NEED -->|Product defect| SUPPORT[Use Support to Product route]
    NEED -->|Commercial scope| ACCOUNT[Use Account or Services route]
    SME --> COORD[Keep one coordinator and decision log]
    SPONSOR --> COORD
    MGMT --> COORD
    SUPPORT --> COORD
    ACCOUNT --> COORD
    COORD --> CLOSE[Decision, action, checkpoint, and residual risk]
```
...
stateDiagram-v2
    ...
    Discovery --> DesiredOutcomes
    DesiredOutcomes --> Baseline
    Baseline --> SuccessPlan
    SuccessPlan --> Governance
    Governance --> Adoption
    Adoption --> ValueReview
    Governance --> Incident: Service event occurs
    Incident --> Learning
    Learning --> SuccessPlan
    ValueReview --> RenewalContext
    RenewalContext --> ContinualImprovement
...
```mermaid
flowchart TD
    OUT[Desired customer outcome] --> BASE[Verified baseline]
    BASE --> GAP[Gap and constraints]
    GAP --> MILE[Milestones and measures]
    MILE --> ACT[Actions and recommendations]
    ACT --> OWN[Owners, dates, and dependencies]
    OWN --> GOV[Governance and escalation]
    GOV --> PROOF[Validation evidence]
    PROOF --> VALUE[Outcome and value review]
    VALUE --> IMPROVE[Continual improvement]
    IMPROVE --> OUT
```
...
```mermaid
flowchart TD
    DATA[Collect scoped current evidence] --> QUALITY[Check identity, freshness, completeness, and provenance]
    QUALITY --> DIM[Assess each health dimension]
    DIM --> CONFLICT{Signals conflict or evidence missing?}
    CONFLICT -->|Yes| UNC[State uncertainty and validation action]
    CONFLICT -->|No| MATERIAL[Identify material exceptions]
    UNC --> MATERIAL
    MATERIAL --> CONTEXT[Apply customer outcomes, criticality, and time horizon]
    CONTEXT --> NARR[Write health narrative and confidence]
    NARR --> ACTION[Prioritize action, owner, date, and validation]
    ACTION --> REVIEW[Review and challenge with stakeholders]
```
...
```mermaid
flowchart LR
    ACT[Activity: analyze and meet] --> OUT[Output: map, report, recommendation]
    OUT --> DEC[Customer decision]
    DEC --> ADOPT[Adopted action or capability]
    ADOPT --> TECH[Technical outcome]
    TECH --> BUS[Business impact]
    BUS --> LEARN[Evidence and learning]
    LEARN -.improves.-> ACT
```
...
```mermaid
flowchart TB
    EXP[Documented exposure before action] --> ACTION[Control or remediation adopted]
    ACTION --> VERIFY[Control validated]
    VERIFY --> OBS[Observed period without the feared event]
    EXP -.unobservable alternate future.-> CF[What would have happened without action?]
    CF -.cannot normally be proved.-> CLAIM[Do not claim a definite outage was avoided]
    OBS --> SAFE[Claim reduced documented exposure and validated control]
```
...
```mermaid
flowchart TB
    ASYNC[Asynchronous evidence and action updates]
    WORK[Working session: resolve technical detail]
    OPS[Operational review: health, cases, risks, actions]
    STRAT[Strategic or business review: outcomes, roadmap, value]
    SPONSOR[Executive checkpoint: material decisions and barriers]
    INCIDENT[Incident governance: restoration and timed updates]
    ASYNC --> WORK
    WORK --> OPS
    OPS --> STRAT
    STRAT --> SPONSOR
    INCIDENT --> OPS
    SPONSOR -.direction.-> OPS
    OPS -.priorities.-> WORK
```
...
sequenceDiagram
    ...
    participant TA as Technical Analyst
    participant LT as Lead TAM
    participant OWN as Customer owners
    participant SME as Support and SMEs
    participant SP as Sponsor

    TA->>TA: Set purpose, data cutoff, and decision asks
    TA->>OWN: Validate actions, environment changes, and constraints
    TA->>SME: Review material technical conclusions
    TA->>LT: Dry run narrative, boundaries, and escalation points
    TA-->>OWN: Send concise pre-read with questions
    LT->>OWN: Open meeting with outcomes and priorities
    TA->>OWN: Present evidence, options, and exact asks
    OWN-->>LT: Decide, assign, defer, or challenge
    LT->>SP: Seek sponsor decision only where authority is needed
    TA-->>OWN: Send decisions, actions, owners, dates, and dissent
    TA->>TA: Track closure and improve next cycle
```
...
```mermaid
flowchart TD
    SHARED[Restate shared outcome] --> LISTEN[Listen for interests and constraints]
    LISTEN --> EVID[Agree facts, uncertainty, and disagreement]
    EVID --> OPTIONS[Create options and tradeoffs]
    OPTIONS --> TEST[Test feasibility, authority, and dependencies]
    TEST --> DECIDE[Decision by authorized owner]
    DECIDE --> RECORD[Record commitment or accepted risk]
    RECORD --> FOLLOW[Follow through and validate]
    FOLLOW --> TRUST[Preserve trust through reliability]
```
...
```mermaid
quadrantChart
    title Illustrative option discussion, not a NetApp method
    x-axis Lower customer effort --> Higher customer effort
    y-axis Lower risk reduction --> Higher risk reduction
    quadrant-1 Strategic investment
    quadrant-2 Efficient control
    quadrant-3 Monitor or accept
    quadrant-4 High effort, limited benefit
    Evidence validation: [0.28, 0.52]
    Phased remediation: [0.58, 0.76]
    Full modernization: [0.88, 0.91]
    No action with monitoring: [0.20, 0.18]
```
...
stateDiagram-v2
    ...
    Recommendation --> Clarification: Evidence challenged
    Clarification --> Recommendation: Revised evidence
    Recommendation --> Deferred: Constraint dominates
    Recommendation --> Declined: Owner chooses alternative
    Accepted --> InProgress
    InProgress --> Blocked
    Blocked --> Escalated: Authority or material risk needed
    Escalated --> InProgress: Barrier resolved
    InProgress --> Validated
    Deferred --> Review: Review date reached
    Declined --> RiskAccepted: Authorized decision recorded
    Validated --> [*]
    RiskAccepted --> [*]
```
...
```mermaid
flowchart TB
    ASK[Customer need or request] --> SCOPE[What is in and out of scope?]
    SCOPE --> OWN[Who owns each action and decision?]
    OWN --> DEP[What dependencies and conditions exist?]
    DEP --> COMMIT[What can be committed, by when?]
    COMMIT --> UPDATE[When will status be updated?]
    UPDATE --> VALID[What proves completion?]
    VALID --> ASK
```
...
flowchart TB
    E[Executive layer]
    M[Management and decision layer]
    T[Technical workstream layer]
    E -->|Impact, risk, decision, checkpoint| M
    M -->|Priority, owner, constraints| T
    T -->|Evidence, hypothesis, action, result| M
    M -->|Verified summary and options| E
```
...
```mermaid
flowchart TB
    USERS[Claims staff and members] --> APP[Claims and document applications]
    APP --> VM[Virtualized compute]
    VM --> PATH[Ethernet and storage connectivity]
    PATH --> DATA[Primary data services]
    DATA --> LOCAL[Local recovery points]
    DATA --> REMOTE[Remote recovery copy]
    DATA --> BACKUP[Separate backup process]
    GOV[Change, risk, support, and account governance] -.controls.-> APP
    GOV -.controls.-> VM
    GOV -.controls.-> DATA
    GOV -.controls.-> REMOTE
```
