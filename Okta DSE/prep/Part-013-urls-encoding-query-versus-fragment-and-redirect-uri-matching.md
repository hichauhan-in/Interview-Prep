...
A user registers as `arti+test@example.com`. The application puts that into a URL parameter without encoding it. The receiving system decodes `+` as a space, and the value becomes `arti test@example.com` — a different, invalid address.
