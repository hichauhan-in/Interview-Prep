...
```mermaid
flowchart TD
    B["'Tell me about a time\u2026'"] --> T["\ud83d\udd35 What is being tested"]
    T --> T1["Did you actually do this?"]
    T1 --> T1a["Specific detail is the evidence"]
    T --> T2["How do you think about it?"]
    T2 --> T2a["\ud83d\udd35 The reasoning matters more than the outcome"]
    T --> T3["What did you learn?"]
    T --> T4["How do you talk about other people?"]
    T4 --> T4a["\ud83d\udd34 Blame is the strongest negative signal"]
    T --> T5["Do your values match ours?"]
    B --> N["\ud83d\udd34 What is NOT being tested"]
    N --> N1["Whether the story ended well"]
    N1 --> N1a["\ud83d\udd35 A failure told honestly beats a success told vaguely"]
    T2a --> R["\ud83d\udd35 A vague success is worth less than a specific failure"]
```
...
flowchart TD
    S["STAR"] --> S1["Situation"]
    S1 --> S1a["\ud83d\udd35 15 seconds. Context only."]
    S --> S2["Task"]
    S2 --> S2a["\ud83d\udd35 10 seconds. YOUR responsibility."]
    S --> S3["Action"]
    S3 --> S3a["\ud83d\udd35 45-60 seconds. THE ANSWER."]
    S3a --> S3b["\ud83d\udd35 'I', not 'we'"]
    S --> S4["Result"]
    S4 --> S4a["\ud83d\udd35 15 seconds. Outcome AND learning."]
    S1a --> M["\ud83d\udd34 Common misuse"]
    M --> M1["\ud83d\udd34 Two minutes of Situation"]
    M1 --> M1a["Interviewer lost before the Action"]
    M --> M2["\ud83d\udd34 'We' throughout"]
    M2 --> M2a["\ud83d\udd34 Cannot tell what YOU did"]
    M --> M3["\ud83d\udd34 No Result"]
    M3 --> M3a["Story just stops"]
```
...
```mermaid
flowchart TD
    C["Your story set"] --> C1["1. A difficult customer situation"]
    C --> C2["2. A complex problem you diagnosed"]
    C --> C3["3. A time you were WRONG"]
    C3 --> C3a["\ud83d\udd35 The most valuable one to have ready"]
    C --> C4["4. A time you said no"]
    C --> C5["5. Something nobody asked you to do"]
    C --> C6["6. A disagreement handled well"]
    C --> C7["7. Learning something fast under pressure"]
    C1 --> X["\ud83d\udd35 Each covers several questions"]
    X --> X1["'Difficult customer' also answers pressure, communication, conflict"]
    X --> X2["'Complex diagnosis' also answers ownership, method, persistence"]
```
...
```mermaid
flowchart TD
    W["'Tell me about a time you were wrong'"] --> F["\ud83d\udd34 Failure modes"]
    F --> F1["A fake weakness"]
    F1 --> F1a["\ud83d\udd34 'I care too much' - transparent"]
    F --> F2["Something trivial"]
    F2 --> F2a["Reads as evasion"]
    F --> F3["Something disqualifying"]
    F3 --> F3a["A security or integrity failure"]
    F --> F4["Wrong, but really someone else's fault"]
    F4 --> F4a["\ud83d\udd34 Worse than not answering"]
    W --> G["\u2705 What works"]
    G --> G1["A real technical misdiagnosis"]
    G1 --> G1a["\ud83d\udd35 Consequential but recoverable"]
    G --> G2["What you noticed and when"]
    G --> G3["\ud83d\udd35 That you said so promptly"]
    G3 --> G3a["The correction is the story"]
    G --> G4["What changed in your method"]
    G3a --> R["\ud83d\udd35 Being wrong is normal. Correcting fast is the signal."]
```
...
```mermaid
flowchart TD
    G["The gap question"] --> S1["1. State it plainly"]
    S1 --> S1a["\ud83d\udd35 No apology, no hedging"]
    G --> S2["2. What you DID about it"]
    S2 --> S2a["Specific labs, specific artefacts"]
    G --> S3["3. What you bring instead"]
    S3 --> S3a["\ud83d\udd35 The substrate, not the product"]
    G --> S4["4. The honest boundary"]
    S4 --> S4a["\ud83d\udd35 Naming what preparation cannot fix"]
    S4a --> R["\ud83d\udd35 Step 4 is what makes steps 1-3 credible"]
```
...
```mermaid
flowchart TD
    C["Closing well"] --> C1["\ud83d\udd35 Ask real questions"]
    C1 --> C1a["Three, prepared, that gather information"]
    C --> C2["\ud83d\udd35 Address anything left hanging"]
    C2 --> C2a["'Earlier I gave a weak answer on X - may I add?'"]
    C --> C3["\ud83d\udd35 State interest plainly"]
    C3 --> C3a["Once, briefly, sincerely"]
    C --> C4["Ask about the process"]
    C4 --> C4a["Timeline and next step"]
    C --> N["\ud83d\udd34 What not to do"]
    N --> N1["'No, I think you covered everything'"]
    N1 --> N1a["\ud83d\udd34 Reads as disengaged"]
    N --> N2["Salary, unprompted"]
    N --> N3["Five questions when there is time for two"]
    C2a --> R["\ud83d\udd35 The recovery is rarely used and lands well"]
```
...
```mermaid
flowchart TD
    Q["Your questions"] --> Q1["'How does someone get recognised?'"]
    Q1 --> A1["\u2705 Specific examples of non-ticket work rewarded"]
    Q1 --> B1["\ud83d\udd34 Only volume and CSAT named"]
    B1 --> B1a["Write-ups will be unpaid overtime"]
    Q --> Q2["'What happens when someone says no on security grounds?'"]
    Q2 --> A2["\u2705 A concrete story where it was backed"]
    Q2 --> B2["\ud83d\udd34 Hesitation, or 'it depends on the customer'"]
    Q --> Q3["'How do support findings reach product?'"]
    Q3 --> A3["\u2705 A named channel and a recent example"]
    Q3 --> B3["\ud83d\udd34 'You can file a bug'"]
    B3 --> B3a["The leverage work does not land"]
    A2 --> R["\ud83d\udd35 A candid negative answer is a GOOD sign"]
```
...
```mermaid
flowchart TD
    W["Final week"] --> W1["\ud83d\udd34 Do NOT learn new material"]
    W1 --> W1a["Shallow and displaces consolidation"]
    W --> W2["\u2705 Drill what you know, aloud"]
    W --> W3["\u2705 Rehearse the four high-probability answers"]
    W3 --> W3a["Protocol \u00b7 gap \u00b7 why this role \u00b7 90 days"]
    W --> W4["\u2705 Re-verify product specifics"]
    W4 --> W4a["\ud83d\udd35 Product docs change - Part 127"]
    W --> W5["\u2705 Re-read your own labs"]
    W5 --> W5a["Your artefacts are your evidence"]
    W --> W6["\u2705 Check the remote setup"]
    W --> W7["\u2705 Sleep"]
    W7 --> W7a["\ud83d\udd35 Outperforms one more study session"]
```
...
```mermaid
flowchart TD
    A["Unexpected behavioural question"] --> B{"Does one of your seven stories fit\nwith a shift of emphasis?"}
    B -->|Yes| C["\u2705 Use it. Re-frame the Action toward what is asked."]
    B -->|No| D{"Do you have ANY real example?"}
    D -->|Yes| E["Use it, even if imperfect"]
    E --> E1["\ud83d\udd35 Real and imperfect beats polished and invented"]
    D -->|No| F["\u2705 Say so, then answer the underlying question"]
    F --> F1["'I haven't hit that exactly. The closest is\u2026'"]
    F1 --> F2["Or: 'Here is how I would approach it, and why'"]
    C --> G["Keep proportions: Action longest"]
    E1 --> G
    F2 --> G
    G --> H{"Did you drift or lose the thread?"}
    H -->|Yes| I["\u2705 'Let me bring that back to the question'"]
    H -->|No| J["\u2705 Stop. Invite the follow-up."]
```
